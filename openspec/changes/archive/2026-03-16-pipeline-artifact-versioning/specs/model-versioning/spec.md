...
### Requirement: 多次訓練不覆蓋歷史版本
**Reason**: 改用 hash-based 版本 ID 後，相同參數 + 相同 dataset 的重跑會覆蓋同一版本（設計意圖）。不同參數或不同 dataset 仍會產生不同版本。
**Migration**: 改為「相同參數和 dataset 產生相同版本 ID」scenario。
