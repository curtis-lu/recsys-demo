...
evaluation/metrics.py SHALL provide `compute_all_metrics(predictions, labels, k_values=[3,5])` that:
...
2. Groups by (snap_date, cust_id) as query groups
3. Computes all 5 metrics for each query
4. Aggregates results into overall, per_product, per_segment (if segment column present), and per_product_segment dimensions
5. Computes macro average and micro average for each dimension

#### Scenario: Overall metrics computed
- **WHEN** predictions and labels are provided with matching keys
- **THEN** result["overall"] contains keys: map, ndcg, ndcg@3, ndcg@5, precision@3, precision@5, recall@3, recall@5, mrr
