...
**Reason**: Holding combo is now treated as an external segment source, loaded via `load_and_join_segment_sources` and analyzed uniformly via `compute_segment_metrics`. The specialized `compute_holding_combo_metrics` function is redundant.
**Migration**: Use `load_and_join_segment_sources` to join holding combo data, then call `compute_segment_metrics(segment_column="holding_combo")`.

### Requirement: Holding combo charts
**Reason**: `plot_holding_combo_charts` is redundant since `plot_segment_charts` handles all segment types uniformly.
**Migration**: Use `plot_segment_charts` with the result from `compute_segment_metrics(segment_column="holding_combo")`.
