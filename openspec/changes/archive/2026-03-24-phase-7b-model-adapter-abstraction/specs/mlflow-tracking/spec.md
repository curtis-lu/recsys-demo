...
#### Scenario: 透過 adapter 記錄模型
- **WHEN** log_experiment 需要記錄模型 artifact
- **THEN** SHALL 呼叫 `model.log_to_mlflow()` 而非直接呼叫 `mlflow.lightgbm.log_model()`

### Requirement: 實驗結果可查詢
MLflow 記錄的每次訓練 run SHALL 包含足夠資訊以比較不同實驗：超參數、mAP 指標、模型 artifact、algorithm 名稱。

#### Scenario: 比較多次實驗
- **WHEN** 執行兩次 training pipeline（不同超參數設定）
- **THEN** MLflow UI 中 SHALL 可查看並比較兩次 run 的 params 與 metrics

#### Scenario: algorithm 記錄
- **WHEN** 查看 MLflow run params
- **THEN** SHALL 包含 `algorithm` param 記錄使用的演算法名稱
