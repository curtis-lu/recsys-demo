...
透過 SQL 將上游資料整理成模型訓練所需的三張來源表：`feature_table`、`label_table`、`sample_pool`。

- 分別提供 `feature_etl`、`label_etl`、`sample_pool_etl` 三條獨立流程。
...
| 分群欄位 | `segment_sources` 指定的 Hive table | 補入客群等欄位，計算 per-segment 指標 |
...
同時調整 `conf/base/parameters_{feature,label,sample_pool}_etl.yaml` 的 SQL 執行順序、partition、primary key 與品質檢查。如果更換 Hive database、來源表名或下游產物名稱，再修改 `conf/base/catalog.yaml`。
