...
| node | 輸入 | 主要功能 | 產出 |
|---|---|---|---|
| `validate_data_consistency` | `sample_pool`、`label_table`、`feature_table` | 抽樣前先檢查 item 集合與 parameters.yaml 設定一致（B1），且 `categorical_columns` 宣告的欄在 `feature_table` 不是連續數值型（B5） | —（擋錯） |
...
| `training` | <ul><li><code>conf/base/parameters_training.yaml</code>：<code>objective</code>、HPO <code>search_space</code>、<code>calibration</code>、<code>sample_weights</code>、<code>hpo_checkpointing</code>（HPO 崩潰復原，見 <code>docs/operations/hpo-resume.md</code>）</li><li><code>catalog.yaml</code>：model 等產物路徑</li></ul> |
...
在抽樣 / 前處理**之前**，先做兩類檢查；失敗會 raise `DataConsistencyError`（所有問題收齊一次列出）。前三列比對「設定宣告的產品」與「資料實際出現的產品」（B1）；最後一列檢查 `categorical_columns` 宣告的欄在 `feature_table` 的實際型別（B5，只讀 schema metadata、不掃資料）。
...
| `categorical column ... is a continuous-numeric type (type=decimal/double/float)` | 你把連續數值欄誤列進 `categorical_columns`；decimal 還會在前處理存檔時炸 JSON 序列化 | 數值特徵就移出 `categorical_columns`（會走數值路徑自動轉 float32）；不該當特徵就加進 `drop_columns` |
