...
- **方法可選**：只訓練一個共用的 LightGBM 模型，訓練目標可切換 —— pointwise（預設 `binary`）或 learning-to-rank（`lambdarank`、`rank_xendcg`）。設定見 `conf/base/parameters_training.yaml`。進階可選模式 `model_structure: per_group_plus_rank` 提供 two-stage stacking（per-grouping Stage-1 + LTR Stage-2），適合處理冷熱門 item 不平衡，預設維持單一模型（`shared`）。
...
指令格式一律是 `python -m recsys_tfb <pipeline> [選項]`，`<pipeline>` 直接寫指令名，如 `dataset`、`training`。`--env` 選擇環境設定。目前 `conf/` 只有 `base/`，可依需求設定 `local` / `production` 要 overlay 的版本，由 `--env` 控制。

### Pipeline 總覽

整條流程分兩種機制：三個 `*_etl` 指令走 SQL，其餘是 Kedro 風格的 DAG pipeline。`evaluation` 有兩個使用情境 —— 訓練後直接評（post-training），或上線 `inference` 後做監控。
...
`source_etl` 走 SQL、非 Kedro 風格 DAG 節點；其餘為 DAG pipeline，逐一列出節點與主要功能。

**source_etl**

`feature_etl`、`label_etl`、`sample_pool_etl` 可以各自串接多支 SQL，一份 SQL 為一張表，表的相依順序定義於 *_etl.yaml 中。
每張表需設定 `partition_by`，依 `partition_by` 做 `INSERT OVERWRITE`，最終必須產出 `feature_table` / `label_table` / `sample_pool`。
建議先用 `feature_etl --source-check` 對上游做唯讀 preflight（全部跑完、有失敗即 exit 1 並印修復指引），通過後再不帶旗標實際寫表；輸出品質檢查失敗會 fail-fast 中止整個 run。

**dataset**

資料集切分區分為：`train`, `train_dev`, `calibration`(可選),`val`,`test`。其角色定位為：
- `train`: 訓練用。
- `train_dev`: early stopping 用。
- `calibration`: 機率校準用。
- `val`: HPO調參用。
- `test`: 評估用。
...
| node | 輸入 | 主要功能 | 產出 |
|---|---|---|---|
| `validate_data_consistency` | `sample_pool`、`label_table` | 抽樣前先檢查 item 集合與 parameters.yaml 設定一致 | —（擋錯） |
| `select_*_keys` / `split_train_keys` | `sample_pool` | 依 sample_group_keys 抽樣，挑各 split 要用的 key（`time, entity, item` 三欄組合） | `*_keys` |
...
| `build_model_input` | `*_keys`、`preprocessed_feature_table`、`label_table` | keys join 特徵、再 join label，組各 split 訓練輸入 | `train/train_dev_model_input`、`val/test_model_input_unfiltered` |
| `filter_groups_with_positives` | `val/test_model_input_unfiltered` | 丟掉 val/test 中沒有任何正例的查詢群組（這種 group 對排序指標 mAP 沒有意義） | `val/test_model_input` |

> `train`、`train_dev` 是訓練資料的依設定於 train_dev_ratio 的比例切分（時間內樣本），HPO 與最終訓練都會用到。
...
| `tune_hyperparameters` | train / train_dev / val 的 parquet handle | Optuna 調參（HPO）— 僅 `model_structure: shared` | `best_params`、HPO 模型 |
| `finalize_model` | train / train_dev 快取、HPO 模型 | 產出最終 LightGBM 模型 — 僅 `model_structure: shared` | `model`（model.txt） |
| `train_composite_model` | 各 split 快取、`preprocessor_metadata` | **`model_structure: per_group_plus_rank` 時**取代 `prepare_lgb_train_inputs → tune_hyperparameters → finalize_model` 三節點鏈；編排 OOF K-fold Stage-1 + Stage-2 lambdarank，產出 `CompositeModelAdapter` | `model`（N+1 booster + grouping 表 + model_meta.json） |
| `calibrate_model`（可選） | 模型、calibration 快取 | 機率校準（如 sigmoid）— `per_group_plus_rank` 下預設關閉 | 校準後 `model` |
| `predict_and_write_test_predictions` | `model`、test 快取 | 對 test set 評分並寫 Hive | `training_eval_predictions` |
| `compute_*` 診斷 | `model`、特徵 | 特徵統計 / 原生 importance / SHAP（composite 下 SHAP 跳過） | 診斷 JSON |

> 為什麼需要 parquet handle ？
>
> `dataset` pipeline 產出的 `*_model_input` 是 Hive / Spark table；但 `training`
> pipeline 後面的 LightGBM、Optuna、diagnostics 主要在 driver 端用 pandas /
> pyarrow 反覆讀資料，不適合每個 trial 都重新掃 Hive。cache node 會先把各
> split 複製成 driver-local parquet，然後只把「這份 parquet 在哪裡」包成
> `ParquetHandle` 傳給後續 node。也就是說，handle 不是另一份資料，而是一張
> 指路牌：讓調參、最終訓練、校準、test 預測都讀同一份本機快取，避免重複
> Spark/Hive I/O，也讓 DAG 裡的資料流保持清楚。
...
- `scripts/sampling_overrides_editor.py`：
    - 將抽樣母體資料表名輸入給子指令 `profile` 根據 `sample_group_keys` 計算分群正樣本數、正樣本率等相關統計數值，輸出為一份 .html 檔。可以在瀏覽器調整後輸出 YAML 片段，決定 `parameters_dataset.yaml` 的 `sample_ratio_overrides` 與 `parameters_training.yaml` 的 `sample_weights`。
    - e.g. ` python scripts/sampling_overrides_editor.py profile <sample_pool_table_name> `
- `scripts/suggest_categorical_cols.py`
    - 從給定的資料推斷 categorical 欄位有哪些，可放進 `parameters_dataset.yaml` 的 `prepare_model_input.categorical_columns`。
    - `--max-cardinality` 參數可以將數值欄位不同值數量小於或等於此門檻時視為類別欄，預設為20。目的是人工檢核欄位是否真的是數值資料，避免模型誤以為數字有連續距離。
    - e.g. ` python scripts/suggest_categorical_cols.py <feature_table_name> --max-cardinality 30 `

### 版本對齊

這裡的版本是 **pipeline 產物的 8 碼 hash**，不是套件版本。用途是讓資料、前處理器、模型與預測結果能互相對應，避免拿錯版本混用；相同的版本輸入會得到相同版本號。

| 版本 | 什麼改動會換版 | 主要對應產物 |
|---|---|---|
| `base_dataset_version` | 非抽樣的 dataset 設定（日期、前處理、`carry_columns` 等）、schema、`feature_table` 欄名／型別／順序 | 前處理器、val/test、共用特徵表 |
| `train_variant_id` | train 抽樣比例、分層規則、`train_dev_ratio` | train/train_dev 資料 |
| `calibration_variant_id` | calibration 抽樣比例、分層規則 | calibration 資料 |
| `model_version` | 上述資料版本，或 `training:` 內會影響模型的設定（演算法、超參數、HPO、校準、sample weights 等） | 模型、test 預測、inference／evaluation 結果 |

因此，改 train 抽樣只會換 `train_variant_id`，不會讓共用前處理與 val/test 重算；但下游 `model_version` 仍會跟著更新。只改執行環境或觀測設定，例如 training 的 `spark`、`mlflow`、`cache`、`diagnostics`，以及 `verbosity`、`log_period`、`num_threads`，不會改 `model_version`。inference／evaluation 的日期、報表設定也不會產生新模型版本。

> 版本 hash 目前不包含程式碼內容或來源表的資料值；若只修程式碼，或同一 `snap_date` 的來源資料被回補但 schema 不變，版本號不會自動改變。這類重跑應特別確認是否會覆寫同版本產物。

各 pipeline 透過 manifest 與 symlink 溝通：

1. `source_etl` 本身沒有上述 hash 版本，以 `snap_date` 維護來源表，供 `dataset` 讀取。
2. `dataset` 成功後寫入各層 `manifest.json`，並把該層的 `latest` 指向本次版本。
3. `training` 預設讀取 `latest` 的 base／train／calibration 版本，也可用 CLI 明確指定；模型 manifest 會記下實際使用的上游版本。
4. 訓練完成**不會自動上線**。用 `scripts/promote_model.py` 將通過評估的 `model_version` 設為 `best`。
5. `inference` 與 `evaluation` 預設讀取 `best`，再由模型 manifest 找回正確的 dataset／前處理版本；輸出以 `model_version` 與 `snap_date` 區分。

使用 evaluation compare 時，CLI 的 `--model-version` 指定本次評估的 Model A（省略則用 `best`）；`--compare <key>` / `--compare-only <key>` 的 `<key>` 是 `parameters_evaluation.yaml` 中 `compare_sources` 的名稱，其中設定的 `model_version` 才是 Model B。兩者彼此獨立，不會互相覆蓋。

簡單記法：`latest` 是「最近成功產生的資料版本」，`best` 是「人工核准上線的模型版本」。
...
| 處理冷熱門 item 不平衡、啟用 two-stage stacking | 手冊 [`gbdt_multiitem_imbalance.md`](docs/handbooks/gbdt_multiitem_imbalance.md)（問題與原理）＋ [`docs/pipelines/training.md §Two-stage stacking`](docs/pipelines/training.md)（config / OOF 資料流 / 節點分支） |
...
3. [`gbdt_multiitem_imbalance.md`](docs/handbooks/gbdt_multiitem_imbalance.md) — 多 item 共享模型的冷熱門不平衡（two-stage stacking 的工程落地見 [`training.md §Two-stage stacking`](docs/pipelines/training.md)）
