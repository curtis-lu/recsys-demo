...
-- 候選＝這一週在這個版位真的發生過的每一次曝光，不是全部 item，也不是「曝光過的
-- 素材」——宣告了 event 之後一次曝光就是一列（#378）。所以每個 query group 的候選
-- 列數因人而異，而且可以超過 item 的種數（銀行示例是每位客戶都配滿全部產品）。
--
...
SELECT
    ...
    l.slot_id,
    l.ad_creative,
    -- schema.columns.event。宣告了角色，sample_pool 與 label_table 都必須帶齊
