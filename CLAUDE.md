...
| 寫或改一個 node 的**內容**（邊界畫哪、body 長什麼樣、機制放哪個檔、命名、讀哪些月份、要不要翻版本） | `docs/agents/pipeline-node-design.md`（跨 pipeline 的判準，18 條裡**只有 3 條有部分機械檢查**——上一列那份擋得住位置，這份擋的是內容。界外違例登記在其〈已登記的例外〉，要加一筆先問使用者） |
