...
**rank**:
一筆候選在所屬 query group 內依 score 由高到低的名次，從 1 起算。
...
**日期區間**:
time 設定的另一種寫法：起日、迄日、間隔（`{start, end, step}`），含頭含尾。設定一載入就展開成它代表的 time 值清單，所以與逐一列出是同一份設定、同一個版本 ID。精確規則：`src/recsys_tfb/core/date_ranges.py` 模組 docstring。
