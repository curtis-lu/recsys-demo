...
**item**（ADR-0027）:
被排序的東西，由一欄或多欄組成。多欄時，各欄值的組合就是一個 item：框架讀使用者的表時把各欄的值用 `-` 拼成一欄、欄名固定叫 `item`，之後只看這個組合；框架自己寫的表與 conf 裡的值都用拼好的值。
...
**rank**:
一筆候選在所屬 query group 內依 score 由高到低的名次，從 1 起算。score 相同時按 item 由小到大排，item 也相同時再按 event 各欄由小到大排（都照欄位本身的值比，數字就照數字大小；item 由多欄組成時照拼好的字串比，不逐欄比）；這只讓名次可重現，不代表模型分得出高下。
...
**特徵表**:
提供特徵的來源表，照接到候選列上的鍵分成兩類：**entity 層級特徵表**與**候選層級特徵表**（ADR-0026）。特徵有沒有偷看未來是使用者來源 SQL 的責任，框架不檢查（ADR-0022）。

**entity 層級特徵表**:
以 base key 接到候選列上的特徵表，一列是某個 entity 在某個時段的特徵；一個部署恰好一張。
_Avoid_: 批次特徵表、每日特徵表（分類看的是鍵，不是多久算一次）

**候選層級特徵表**:
以 identity 接到候選列上的特徵表，一列是一筆候選的特徵，例如曝光前 30 分鐘的瀏覽次數；選用，最多一張。宣告了它，離線推論就不能用。
_Avoid_: 即時特徵表（框架看不到新鮮度，不即時但以 identity 接的表也屬於這一類）、事件層級特徵表（會和 **event** 混淆）
...
**dataset 產物格式版本**:
框架自己的一個整數，進 base_dataset_version 的雜湊。程式改了 dataset 落地的內容、設定卻沒動時加 1，讓每個部署的版本身分都換一次。精確定義：`DATASET_ARTIFACT_FORMAT_VERSION`（同上）。
...
**全正的 query group**（#376）:
裡面每一筆候選的 label 都是正例的 query group；label 有分級時，指每一筆都是同一個正值。不管怎麼排，它的組層級排序指標都一樣，排法分不出好壞。evaluation 預設照樣算進衡量指標，只印出它的佔比；`evaluation.query_filter.drop_all_positive_groups` 打開時才排除。
