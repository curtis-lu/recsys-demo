...
def test_train_set_excludes_val_and_test_dates(train_set):
    """train_model_input 的 snap_dates 不應包含 val (2025-06-30) 和 test (2025-07-31)。"""
    snap_dates = set(train_set["snap_date"].dt.strftime("%Y-%m-%d").unique())
    assert "2025-06-30" not in snap_dates, f"train 不應含 2025-06-30，實際: {snap_dates}"
    assert "2025-07-31" not in snap_dates, f"train 不應含 2025-07-31，實際: {snap_dates}"


def test_train_dev_shares_dates_with_train(train_set, train_dev_set):
    """train_dev 的 snap_dates 應為 train 日期的子集（共用日期，按 cust_id 切分）。"""
    ...
    assert dev_dates.issubset(train_dates), (
        f"train_dev 日期 {dev_dates} 應為 train 日期 {train_dates} 的子集"
    )


def test_val_snap_dates(val_set):
    """val_model_input 的 snap_dates 應恰為 [2025-06-30]。"""
...
def test_all_splits_nonempty(train_set, train_dev_set, val_set):
    """各 split 行數應大於零。"""
    assert len(train_set) > 0, "train 為空"
    assert len(train_dev_set) > 0, "train_dev 為空"
    assert len(val_set) > 0, "val 為空"
