...
| **對 loss 做特徵歸因** | 這些錯是**哪些特徵**造成的（單列層級） | TreeSHAP 的 `model_output='log_loss'`（Lundberg et al., 2020） | 確立、但用熟的人少 |
...
| loss 歸因＋錯誤分群（單列層） | `shap` 的 `model_output='log_loss'` ＋ 一般分群工具（sliceline / pandas groupby）可組出；本題的訓練 loss 恰好就是 binary logloss，單列層無新數學 | 處方層（歸因→建議動作）無現成件，見 §9.6 的斷層分析 |
...
- **`model_output='log_loss'`**：歸因對象從分數換成單列 loss，額外需要**標籤**。interventional + log_loss 的組合，用在本題的單列層是正確的選擇。
...
5. **單列層 loss 歸因要不要做成獨立可重用工具：未定。** 本題先把指標層（替換實驗、成對帳本）做進 pipeline；單列層若日後要做，再決定形式。**代價**：現在不必想清楚，但實作時別把單列層歸因寫死進 pipeline 內部——留可抽出的接縫，否則日後想獨立化要重寫。
