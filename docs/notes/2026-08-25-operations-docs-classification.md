...
> 範圍後來從 `docs/operations/` 外溢到相鄰目錄——同一把「依讀者分」的尺，撞到哪份就處理哪份。

| 日期 | 做了什麼 |
|---|---|
| 2026-08-25 | `docs/handbooks/handbook-writing-guide.md` → `docs/agents/`。它的讀者是**寫手冊的人**，不是讀手冊的人，跟 `diagnosis-report-presentation.md` 同型；`docs/handbooks/` 因此回歸純成品（`gbdt/`、`spark-tuning/`）。查證重疊：`writing-technical-handbooks` skill 的 `references/writing-style.md`（68 行）就是從這份泛化去數學味來的，§1–9 全被收走；**skill 沒有的是 §10 數學排版體例與 §12 的具體案例**（`$$\boxed{}$$`、校準機率、RankNet、leaf-wise、proper scoring rule，關鍵字對照 skill 命中皆為 0），所以不是重複而是分層——已在檔頭寫明分工。連帶改：spark-tuning 草稿的 TODO、**`~/.claude/rules/00-core.md:101`**（全域常駐規則，repo 外；改前已驗證新路徑存在）。 |
