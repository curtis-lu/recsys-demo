...
> **2026-09-20 註**：各條的〈撞凍結〉欄引用的 `docs/agents/deliberate-non-goals.md`**已整份刪除**（經過見 `docs/agents/issue-tracker.md`〈票的範圍宣告〉）。那些欄位是 2026-09-09／09-13 當下的判斷，**原文保留不改寫**——它記的是當時的考量。要重新判斷某條修法撞不撞什麼，以現在的 code、ADR 與 `docs/agents/architecture-constraints.md` 為準，不要拿這一欄當現況。
