...
**為什麼產品層是「帶」不是單一數字**：override 的 key 是三維（客群|產品|label），同一個產品在不同客群的保留率不同。ccard_ins 三個客群的偏移分別是 +0.693 / +0.105 / +0.223，混在一起的產品層效應取決於各客群買家佔比（資料相關、config 算不出來），所以理論值只能給範圍：`theory_min=0.105`、`theory_max=0.693`。這是誠實的近似——**帶越寬，verdict 越寬容**（任何落在帶內的實測值都算可解釋），判讀時記得看一眼帶寬。cell 級的精確值在 `reconciliation.json` 的 `theory.cells`。
...
| 欄位 | 意思 | 判讀 |
|---|---|---|
| `theory_min` / `theory_max` | 配置推得的理論偏移帶（§2） | 帶寬＝跨客群異質度；越寬 verdict 越寬容 |
| `gap` | 絕對實測差距（§3） | 含母體效應，**不要**直接拿來判產品好壞 |
| `gap_vs_global` | gap 扣掉全局參考值（§4） | verdict 的依據 |
| `gap_calibrated` | 校準後分數（`score` 欄）的同式 gap | 同樣含母體效應，看下一欄 |
| `gap_calibrated_vs_global` | 校準後 gap 扣掉（校準版）全局參考值 | 校準層有效時，它的量級應明顯小於 `gap_vs_global`、趨近 0 |
| `residual` | `gap_vs_global` 距理論帶的距離（帶內＝0） | \|residual\| ≤ 門檻（config `explained_threshold`，預設 0.3）→ 可解釋 |
| `verdict` | 可解釋／不可解釋／無法評估 | 見下方判讀順序 |
| `reason` | verdict＝無法評估時的原因說明 | 只有無法評估的列才有值 |
| `p_mean` / `y_rate` / `n_rows` | 平均預測機率／實際正類率／列數 | n_rows 小的產品，gap 本身就吵 |
