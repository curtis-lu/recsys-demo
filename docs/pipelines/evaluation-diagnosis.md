...
| `residual` | `gap_vs_global` **超出**理論帶 [theory_min, theory_max] 的部分。落在帶內＝理論全數解釋掉了＝residual 為 0；帶是 [0, 0] 的產品（配置沒動過）沒有任何理論可扣，residual 就恰好等於 gap_vs_global |
...
| `residual` | `gap_vs_global` 超出 [`theory_min`, `theory_max`] 的部分（算法見表下說明） | \|residual\| ≤ 門檻（config `explained_threshold`，預設 0.3）→ 可解釋 |
...
**residual 的算法，用三個例子講清楚**。公式是 `residual = gap_vs_global − clip(gap_vs_global, theory_min, theory_max)`，其中 clip(x, a, b) 是「把 x 壓回 [a, b] 範圍：小於 a 取 a、大於 b 取 b、在範圍內取 x 本身」。直覺：理論帶能解釋的部分先扣掉，剩下的才是 residual。

1. **落在帶內** → residual = 0。ccard_ins：gap_vs_global = +0.329，帶 [0.105, 0.693]，0.329 本來就在範圍內，clip 後不變 → residual = 0.329 − 0.329 = **0**。意思：實測偏移完全在理論預期範圍內，沒有需要另外解釋的量。
2. **帶是 [0, 0]（配置沒動過的產品）** → residual 恰好等於 gap_vs_global。fund_bond（無注入時）：gap_vs_global = −0.186，帶 [0, 0]，任何數 clip 到 [0,0] 都是 0 → residual = −0.186 − 0 = **−0.186**。意思：這產品沒有任何理論偏移可扣，整個 gap_vs_global 都是「理論解釋不了的部分」——所以對帳表上大多數產品的 residual 會**剛好等於** gap_vs_global，這是正常現象不是巧合。

另一個看起來像巧合的數字：**恰有一個中性產品的 gap_vs_global（因而 residual）顯示 0.000**。那是因為全局參考值取的是中性產品 gap 的**中位數**——中性產品有奇數個時，中位數就是排序後正中間那個產品自己的 gap，它減自己當然是 0（本機現狀是 ccard_cash：7 個中性 gap 排序後它排第 4，參考值 −0.462 就是它的 gap）。這代表「它被選為基準」，不代表它特別健康。
3. **超出帶** → residual＝超出最近邊界的量。假設某產品 gap_vs_global = +1.0、帶 [0.105, 0.693]：clip 得 0.693 → residual = 1.0 − 0.693 = **+0.307**，剛好壓過 0.3 門檻 → 不可解釋。
