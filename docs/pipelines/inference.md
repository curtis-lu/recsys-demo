...
| 主要輸入 | `feature_table`、版本化 `preprocessor`、版本化 `model` |
...
4. **評分日期資料已就緒**：`feature_table` 必須包含每個 `inference.snap_dates`。任一日期完全缺少資料時，`build_scoring_dataset` 會立即中止。
5. **Feature identity 唯一**：每個 `time + entity` 在 `feature_table` 中應只有一列。重複列會在 join 後放大評分母體，最後通常被 completeness 或 duplicate check 阻擋。
...
框架會先取得每個日期中 distinct 的 `(time, entity)`，再與 `products` 做 cross join。因此每個 query group 預設具有完全相同的候選集合：
...
目前不支援每個 entity 擁有不同候選集合。若 item 有資格、法遵、庫存或可見性限制，必須在 `build_scoring_dataset` 增加 eligibility 邏輯，不能只依賴模型把不適用 item 排到後面。
...
|---|---|---|---|---|
| 建立母體 | `build_scoring_dataset` | `feature_table`、parameters | 篩日期、取得 distinct entities、與 products cross join、接回 feature columns | `scoring_dataset` |
...
`build_scoring_dataset` 先從 `feature_table` 取出設定日期中的 distinct `time + entity`，再與完整 products 清單做 cross join，最後依 `time + entity` left join 回原始 feature table。

因此：

- item 本身不需要存在於 `feature_table`。
- 每個 entity 預設會得到全部 products。
- feature table 的 `time + entity` 重複列會造成 join fan-out，應在 source ETL 或資料驗收階段先排除。
...
| `feature_table missing inference.snap_dates` | 日期尚未產出、格式錯誤或 source ETL 未完成 | 查 `feature_table` distinct 日期並修正 `snap_dates` |
...
| `no_duplicates` | 同一 identity 重複 | 優先檢查 `feature_table` 的 `time + entity` 唯一性 |
...
- 推論使用的 `feature_table`：[`source_etl.md`](source_etl.md)
