...
train、calibration、val、test 日期集合必須互斥（一致性不變量 A24，在 `dataset` 指令啟動 Spark 前檢查；按日比對，同日的不同寫法也算重疊）。`train_dev_ratio` 不會切日期，而是依第一個 entity 欄位將該 entity 的所有日期與 items 一起分配至 train 或 train-dev，避免同一 entity 同時出現在兩側。
...
| `(A24) dataset.X_snap_dates and dataset.Y_snap_dates both contain [...]` | train/calibration/val/test 使用相同日期 | 重新切分日期，確保集合互斥。此檢查在 Spark 啟動前執行，且**按日比對**——`2026-1-31` 與 `2026-01-31` 算同一個月 |
