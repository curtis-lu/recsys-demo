...
- 日期：2026-06-14
- 狀態：設計已與 user 確認全部決策；文件改於實作 PR 一併更新，待最終 review
...
- **註記**：scoring_dataset（join 後的 in-memory 表）多一個 boolean 欄 `feature_present`，缺特徵的母體成員 = `false`。此欄**僅存在於 in-memory scoring_dataset**，不下推到 Hive 輸出表（見「已確認決策」）。
...
- **`feature_present` 不下推**：只留在 in-memory scoring_dataset + log 回報，**不**寫入 `score_table` / `ranked_predictions` 等 Hive 輸出表（避免輸出表 schema 演化；日後 production 端確有需求再評估）。
