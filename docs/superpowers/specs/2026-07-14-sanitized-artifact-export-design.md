...
2. **刷除**：對文字副檔名（`.json .txt .html .htm .log .md .csv .tsv .yaml .yml`）讀入內容，用 `--id-pattern`（預設 `\b[A-Z][0-9]{9}\b`）做 regex 取代成遮罩值，就地寫回**輸出檔**。
...
3. **二進位檔**（`.png .pkl .parquet .bin` 等非文字）：**原樣複製**，無法 regex（殘餘風險見 §8）。
