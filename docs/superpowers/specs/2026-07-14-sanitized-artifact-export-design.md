...
- **`.png` / `.pkl` 不刷**：診斷圖（shap_summary、waterfall）標題不含 id，但無法程式化保證圖內未渲染 id；`.pkl`（calibrator）為模型參數、不含 id 但也不可 regex。列為殘餘風險——若擔憂特定二進位檔，可在來源端先移除再匯出（診斷需要 png，故不預設排除）。
...
- **檔名不刷**：只刷檔案「內容」，不改**檔名／路徑**。本產出物檔名為索引／象限式（如 `waterfall_high_0.png`），不含 id。若未來有以 id 命名的產物，需另行處理——目前列為已知邊界。（內容中與底線／字母相黏的 id 已因預設 pattern 去 `\b` 錨定而會被刷，見 §5。）
