...
**推論：重建本機 Spark 不需要動 `src/` 也不需要動 `conf/base/`，只要新增一份 `SPARK_CONF_DIR` 並改寫 setup/teardown 與文件。**
...
- **每個 worktree 各自一份 warehouse / metastore_db**（相對路徑 + worktree 各自 CWD），天然隔離，互不污染。
...
- **CLAUDE.md**：刪「Local dev-cluster testing」+「Pipeline 與 SPARK_CONF_DIR 的對應」兩節，換成一節薄的「本機 Spark 測試」指向新 doc。
...
- **`docs/operations/worktree-venv-setup.md`**：補 `local_warehouse` / `metastore_db` 為 worktree 本地產物（不 symlink、各自一份、`--reset` 各自清）。
...
- `CLAUDE.md`（Spark/Hive/dev-cluster 相關節）
...
- `conf/base/`（catalog / parameters；本機路徑全由 `SPARK_CONF_DIR` 注入）
- `~/dev-cluster/`（使用者自行處理）
