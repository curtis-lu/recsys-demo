...
- `log_experiment` 增輸入 `quadrant_profiles`,寫檔前 `shap_diagnostics["per_quadrant"] = quadrant_profiles`(**`compute_shap_diagnostics` 不動**;per_quadrant 併入既有 `shap_diagnostics.json`,對齊 spec §6.3)。
