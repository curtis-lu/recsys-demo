...
隱性移除（**這是本塊的核心**）：以下三處是 key-agnostic 表格，把 metrics dict
的 key 直接攤成欄／列，原始碼裡沒有 `ndcg` 字樣，但 `ndcg@k` 會自動渲染出來：

| 位置 | 機制 |
|---|---|
| `report_builder.py:869` | per-segment：`pd.DataFrame(rows).T` |
| `report_builder.py:940-950` | baseline overall：`sorted(set(a) \| set(b) \| set(delta))` |
| `comparison/report.py:99-118` | comparison overall：`:104` 同上 |
