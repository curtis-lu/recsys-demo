...
2. **分區 fixture(驗證分區重建)**:新增 hive-partitioned 測資(`pyarrow.dataset.write_dataset(partitioning=["snap_date","prod_name"])`),驗證 prod_name **從分區重建**後 `compute_shap_diagnostics` 正常運作(現有 SHAP 測試用 flat parquet,未覆蓋分區路徑,此條補上)。
   - **注意(不變式界定)**:byte-for-byte 不變式是「**同一物理佈局下,新程式 == 舊程式**」,**不是**「flat 輸出 == partitioned 輸出」。後者為**偽不變式**——依 `(snap_date, prod_name)` 分區會**重排列**,`read_column` 取得的 item 順序不同 → `_stratified_item_sample` 抽到不同(但同樣有效)的子集 → 輸出本就不同。真正的不變式由 §8.4 的 leg-by-leg 原語測試(`take_rows == read_table().iloc[idx]`(flat+partitioned)、`read_column` 順序、抽樣 determinism)組合保證,不另立 flat==partitioned 斷言。
3. **行為測試(把重構意圖釘進測試)**:
