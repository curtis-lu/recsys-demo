...
- 新檔 `src/recsys_tfb/diagnosis/metric/reconciliation.py`：
  ...
  - **修訂（2026-07-07，Phase 2 真跑實證）**：verdict 的比較基準＝「gap 相對全局參考值的偏離」而非絕對 gap。證據：post-training 評估母體只含 test 窗有正例的客戶（training_eval_predictions 寫入條件），母體條件化把全體 item 的 gap 一致下移（本機實測 −0.38～−0.44 log-odds、8 item 全中），絕對式 verdict 會把這個共同水準誤判成 per-item 不可解釋。全局參考值＝理論帶為零的 item 的 gap 中位數（配置中性基準，同手冊 divergence_from_global 的思路）；中性 item 不足 3 個 → 退回 0（即原絕對語意）。絕對 gap 與全局參考值照列（全局偏移本身是獨立現象，報表註明成因），只有 verdict 改吃相對值。
...
> **執行時修訂（2026-07-08，Phase 4a 真跑實證，兩點）**：(1) **gauge 讀法**——對所有 item 加同一常數不改變排序、mAP 不變，δ* 只有相對差可識別；座標下降選到的解帶有任意共同平移（實測注入 +1.0 時原始 δ*=−1.40、但去均值後的 centered 位移 −1.05 ≈ −1.0）。產物已加 `delta_star_centered`（去均值），驗收條 1 的「δ*_j ≈ −1.0」以 **centered 的注入前後位移**判讀。(2) **驗收條 2 的前提在合成資料上不成立**——現狀（無注入）折外收復量 ≈ +0.035、δ* 非全零（fund_bond/fund_stock centered ≈ +0.5～0.7）：合成資料對 macro 等權指標存在真實的「指標再平衡型」水準缺口（冷門 fund 系抬高時 per-item AP 大賺、傷害分攤到熱門 item 很小），與 Phase 2「校準水準都在理論帶內」不矛盾（對帳量校準、sweep 量指標最優）。驗收條 2 改讀：清掉注入後產物與注入前的乾淨基準**位元一致**（確定性），而非 δ* 全零。

> **執行時修訂（2026-07-08，Phase 4b 真跑實證）**：**驗收條 1 的 sweep 部分（δ* ≈ −1.0）在 λ=0.1 下對本合成資料不成立，且不成立是工具的正確行為**。定量：注入 ccard_ins +1.0 對 macro per-item mAP 的全樣本實際損傷僅 +0.0034（ccard_ins 自身 AP 因正例上移而變好，抵銷他人損失；fit 折 0.0070、holdout ≈0），小於 δ=−1.0 的收縮懲罰 0.1×1²/8=0.0125——sweep 不反制（centered 位移 +0.04）是收縮語意「不值得修的錯位就不修」的正確輸出。否證實驗：λ=0＋同注入 → sweep 反制（raw δ*=−1.30、centered 位移 −0.881）。回頭看 Phase 4a 的 −1.05「通過」帶折別運氣成分（當時 fit 折損傷同樣僅 0.0040 < 0.0125；座標下降路徑借機在 fit 折找到更高解，其 fit star 0.6039 > 乾淨態 0.5925、折外 star 兩者相同）。**驗收條 1 改讀**：注入的已知答案主判準＝**pair_ledger 壓制帳暴增**（pair 層面錯序不受 λ 影響；實測 dap_sum ×1.98、share 34%→62%）＋三份非分流 JSON 位元不變；sweep 的 δ* 反制只在「損傷 > λ 懲罰」時才可期待，驗收時附損傷 vs 懲罰的數字對照（λ=0 對照跑可選）。
