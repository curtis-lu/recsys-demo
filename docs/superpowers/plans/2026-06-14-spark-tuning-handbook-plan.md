...
> **▶ 目前進度 / 下一步（/compact 後先讀這裡）**：第 01–05 章已寫完、雙 subagent 審＋triage 修＋user glance。第 05 章「儲存效率」骨架＝「§03 花用本錢、§05 把本錢存進去」（格式 Parquet/ORC 欄式→壓縮→partition 設計→**小檔成因/徵兆/解法主體**→ANALYZE 餵統計→bucketing→營運共用表 Hive3 managed/external/HWC→§5.9 貫穿範例），3 圖；經 user 兩輪深問後加料（repartition vs coalesce 平行度、寫分區表帶分區欄位、CTE hint 放最外層、ANALYZE 非自動、external 表 Hue/Impala 可查、dbt-spark、DataNode/locality）並做了**誠實更正**（小碎檔對 locality 影響其實間接偏弱、真正傷的是不可切分大檔）；**全書 `列式`→`欄式`**（繁中欄=column/列=row，03+05 已改）；跑了兩輪雙審，r2 reviewer 連 cbo/autoUpdate 預設都翻 3.3.2 SQLConf 原始碼確認 false、r2 reader 抓出新內容過載→已把 §5.4 locality 改結論先行、§5.5 過載小節拆成兩個 `###`。**下一步＝Task 6 寫第 06 章「引擎選用：Spark vs Hive/Tez vs Impala」**（`06-engine-selection.md`，PRIOR_CHAPTERS=01, 02, 05），照 spec §12 基準＋§6 來源慣例＋§10 審稿流程（worktree `.worktrees/spark-handbook`、分支 `feat/spark-tuning-handbook`）。**體例慣例（01–05 已立）**：precision footer 不放 .reviews 指標；插章中間優先做 `###` 小節避免重編號；能上 mock 面板/貫穿範例就上；環境細節以 user 公司經驗為準；審稿後若再加料要補審（r2 即如此）；commit 後**不必**還原 graphify-out/GRAPH_REPORT.md（已 untracked）。第 06 章須查證重點見下方 Task 6（Impala INVALIDATE/REFRESH、Impala 對 Hive ACID 表支援限制、三引擎定位）。
