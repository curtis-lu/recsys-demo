...
**▶ 下一步（user 2026-06-22 已定）＝寫 Task 9 `10-scenario-playbooks.md`（第 10 章 場景對應＝純索引）。** ch07 PySpark **仍暫緩**（user 跳過它先寫 ch10）。ch09（含 §9.6 進階版本化＋WAP）已寫完＋審＋commit（`8b5cdbc`/`0e6d2d4`/`3622365`）；user 最終 glance 仍開著但不擋 ch10。**ch10 寫法（完整大綱見下方 Task 9）**：純索引/指路、**不重教概念**，把前面各章技巧按三情境（① ad-hoc ② 排程產表 ③ 特徵運算/特徵庫）串成「遇這種工作→照哪些章→最常踩什麼雷」；每情境＝典型流程→對應章節清單→該情境最常踩的雷；1 張總表或三張對照圖。**須查證重點＝無新技術主張，只查跨章引用指對**（§02 診斷/§03 SQL/§04 config/§05 storage/§06 引擎/§08 排程可靠性/§09 品質·時間點·版本）。**⚠️ ch07 未寫的處理**：情境③本來引 §07（DataFrame API），因 07 暫緩→用**軟指標**「複雜可重用/要測試的特徵邏輯→可考慮 DataFrame API（見第 07 章，撰寫中/暫緩）」，**別下會 404 的硬連結**到 07 檔；其餘 §01–06、§08–09 都已寫、可正常連。PRIOR=01–09（07 例外）。Step A 因無新技術主張可從簡，重點是逐一核對 cross-ref 章節號與各章實況一致。**體例慣例（01–09 已立）**：precision footer 不放 .reviews 指標；深主題/插章中間優先做 `###` 小節避免重編號；能上 mock 面板/貫穿範例就上；環境細節以 user 公司經驗為準；**審稿後若再加料一定要補審**；reader 常抓「術語/縮寫詞第一次出現沒當場定義、無 ML 背景讀者缺特徵/label/training/serving 錨點」→第一次承重就 gloss；commit 後**不必**還原 graphify-out/GRAPH_REPORT.md（已 untracked）；**審稿 subagent 長跑易 socket 死→可分段派或關鍵段自審補**。
...
- 場景 3 特徵運算/特徵庫：寬表多 join/window、易 skew、時間點正確性 → 引 §09（時間點/特徵庫契約）+ §03/§05/§08（§07 軟指標，見下方 ⚠️）。
- 每場景：典型流程 → 對應章節清單 → 該情境最常踩的雷。

**⚠️ ch07（PySpark）仍暫緩、尚未寫**：場景 3 本要引 §07（DataFrame API）→用**軟指標**「複雜可重用/要單元測試的特徵邏輯→可考慮 DataFrame API（見第 07 章，撰寫中）」，**不要下會 404 的硬連結**到 `07-pyspark-dataframe-api.md`。其餘 §01–06、§08–09 皆已寫、正常連。本章是純索引、可較快寫；重點在 cross-ref 章節號與各章實況一致（§08＝排程可靠性、§09＝品質/時間點/版本、§06＝引擎選用…）。
