...
- **Back-compat:** existing configs set `sample_weights: {}`, so the default `[prod_name]` causes no behavior change. Note the A9 gate only fires when `sample_weight_keys` is actually present: if a config omitted `sample_weight_keys` but kept a pre-existing `"<seg>|<prod>"` weight table, all three predicates short-circuit (arity 0 / empty key list) and the weight silently no-ops at runtime — the gate does NOT flag this. A9a/A9b/A9c only catch mistakes once `sample_weight_keys` is declared (which the base YAML now always does). The previously-silent column dependency is surfaced for any config that declares keys, not for ones that omit them.
