...
## Task 3: Sparse JSON→YAML conversion with A5/A7 validation (`grid_to_yaml`)
...
def grid_to_yaml(
    ...
    via the A5 / A7 consistency predicates (collect-all) BEFORE returning, so
    ...
    # Reuse the single-source consistency predicates (A5 + A7).
    probe = {**parameters}
...
```bash
GIT add src/recsys_tfb/tooling/sampling_suggest.py tests/test_tooling/test_sampling_suggest.py
GIT commit -m "feat(tooling): grid_to_yaml sparse emit + A5/A7 validation"
```
...
class TestToYamlCli:
    def test_to_yaml_prints_both_blocks(self, tmp_path):
        ...
        # minimal params yaml the command reads for A5/A7
...
Subcommands:
  ...
  to-yaml <json>    Convert the browser JSON export into sparse YAML snippets
                     (A5/A7-validated) for manual paste into config.
...
def to_yaml(
    ...
    params: Path = typer.Option(
        Path("conf/base/parameters_dataset.yaml"), help="params yaml for A5/A7"),
) -> None:
    cfg = yaml.safe_load(params.read_text())
...
- **Spec coverage:** D1 (YAML snippet manual paste) → Tasks 3,6 (`to-yaml` prints, never writes config). D2 (self-contained HTML, sparse export) → Task 4 (no external assets; export filters `!= default`). D3 (data-driven, item axis from schema) → Tasks 2,5 + A5/A7 reuse in Task 3. D4 (train-only, sparse) → Task 3 (`_NEG_LABEL="0"`, sparse emit). D8 formulas → Tasks 1,2. A5/A7 reuse (single source) → Task 3 imports the consistency predicates rather than re-implementing.
