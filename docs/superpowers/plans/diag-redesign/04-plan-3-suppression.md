...
⚠ **`tests/test_diagnosis/test_metric/test_common.py` 目前不存在**（本節初稿誤寫成「追加到」——查證 `git log --all -- <該路徑>` 零紀錄）。這個 task 要**新建**它。

> **順帶暴露的落差，本 Plan 不修**：`_common.py` 的既有函式（`query_key`／`sample_arrays`／`ci_for_corrected_minus_baseline`／`to_logit`／`apply_injection`）**沒有任何直接測試**，全靠消費者間接覆蓋。共用層沒有自己的測試，代表「改壞它」的紅燈只會出現在別人的測試檔裡、訊息也指不到根因。**寫進 Plan 5 收尾清單**，不要在這裡順手補。

新建 `tests/test_diagnosis/test_metric/test_common.py`，內容：
...
**作法**：`axis_order` 在 render 端截到前 `N` 個 item，`N = floor(sqrt(MAX_FIGURE_POINTS))`（＝44）。挑哪 44 個：**依 `overall_ap_gap_share` 由大到小**（即在成對表裡分攤到最多缺口的 item）。

**為什麼是截斷而不是退回表格**：`item_ability/_render.py:130-142` 的既有慣例是超預算就改用表格。那個慣例在**逐 item 一列**的圖上是對的（表格完整、不損失資訊）；但矩陣退回表格會變成 |items|² 列的東西，沒有人讀得下去，而且**兩張圖並排對照**正是這一項診斷的全部價值，拿掉圖等於拿掉診斷。完整資料本來就在成對表裡，所以截斷不損失任何東西——只損失「一眼掃過去」的範圍。

**但說明的位置照既有慣例走**：寫在該 section 的 **`bullets`**（不是 `description`），措辭沿用 `item_ability/_render.py:140-142` 的句型：
...
`item_ability/_render.py:80-83` 有個私有 `_fits(n_points)`，它的 docstring 自陳存在理由是「**與 `figures.assert_within_budget` 同一常數、同一比較方向，避免『這裡判斷畫得下、那裡 raise』的死角**」。在 `suppression/_render.py` 再複製一份，正好破壞它自己說的那個理由——兩份私有副本可以各自漂移。

**改法**：在 `src/recsys_tfb/report/figures.py` 的 `assert_within_budget` 旁邊加一個 `fits_budget(n: int) -> bool`（同常數、同比較方向），`item_ability/_render.py` 與 `suppression/_render.py` 都用它，刪掉 `_fits`。

這是本 task 唯一動到 `item_ability` 的地方，diff 應該是 **−4/+1 行**。動它的理由不是「順手整理」，是那段 docstring 指名要求的東西。
...
def test_axis_is_not_capped_when_items_fit():
    ...
    """
    sections = suppression.render(_result_with_items(22), {})
    supp = _figure_of_type(sections, "heatmap")
...
### Task 5.5: real-run 驗證 ✅ 已完成（2026-07-21）

`--post-training --model-version 6059dcef`，654 queries。**六項全部通過**：

| 檢查 | 結果 |
|---|---|
| node 數 | **14**（`+diagnose_suppression`、`−compute_pair_ledger`），pipeline 50.38s |
| `diagnose_suppression` 耗時 | **0.04s**；`n_pairs=1273`、`n_positive_rows=1006`、`n_misordered_pairs=1273`、8 items |
| 四頁 HTML ＋ index | 01–04 編號連續，四個連結都在，全部是本次寫出（比對 mtime，不是比對「檔案存在」） |
| 四份 JSON | 過**嚴格**解析（`parse_constant` 攔 `NaN`／`Infinity` 字面值——Python 的 json 預設靜默接受，那不是合法 JSON） |
| **會計恆等式在真實資料上成立** | `total_ap_gap_allocated_to_suppressors` 295.46428571428567 vs `total_row_ap_gap_allocated` 295.464285714286 |
| 離線重繪 | 2.6 秒產出四頁 |

**`report.html` 的差異恰好如預期**。用標題集合比對（不用行 diff——這份 HTML 有很多超長單行，difflib 的行對齊會誤導）：移除 4 個標題，**全部屬於 pair_ledger 區塊**（h2「壓制帳本 Pair ledger」＋ h3「壓制者邊際」「傷害 × segment」「Substitution ablation」），**零新增**。其餘只有目錄編號遞補與「本次寫出 3 頁 → 4 頁」。

> **一個把自己絆倒的細節，記下來**：第一次跑驗收時「`pair_ledger.json` 未被本次重寫」這條**失敗**了。原因不是產物有問題，是我拿 `stat -f %B`（birth time）當本次起跑基準——而 `realrun.log` 是用 `>` 覆寫的，**`>` 截斷檔案但保留 inode 與 birth time**，所以抓到的是上一次 real-run 的時間。改用實際時鐘基準後通過。**判準：用檔案時間當基準之前，先確認那個時間戳真的屬於這次執行。**

#### 收工時順手修的兩處措辭（`29952e5`）

1. `SCOPE.blind_to` 原寫「依 **severity** 比例分攤」——`raw_severity` 是內部變數名，洩到使用者面就是看不懂的英文；而且 `severity` 正好是三條鐵則明令不得出現的判定用語，同一個字出現在頁面上會讓讀者以為系統在評級。
2. section 標題「具體案例：被壓制的正例列」下面緊接表格標題「具體案例（依 gap 排序）」，連著讀會結巴。表格標題改成只補 section 沒說的資訊。
