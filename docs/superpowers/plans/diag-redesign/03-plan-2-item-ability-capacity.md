...
> ⚠ **開工前先做「接線層重構」（README 的「下一步」段）。** Task 2.8 的回饋暴露出接線層有已知缺陷，且已在公司環境造成一次實際故障（`generate_report` 位置綁定平移 → 某項診斷的資料被當成另一項處理）。本計畫的 Task 3.x 若照 Plan 1 現有形狀抄，會複製那個缺陷。
>
> 重構完成後，本計畫有三處要改寫，**不要照抄舊形狀**：
> 1. **不再手寫診斷 Node**——Node 由 `contract.DIAGNOSES` 導出，新增診斷只需補 registry ＋ 子套件。
> 2. **不再往 `generate_report` 的 inputs 加位置元素**。
> 3. `_render.py` 的形狀（六節、每節帶 formula ＋ bullets）**照抄**——那部分使用者已驗收通過，沒有變動。
