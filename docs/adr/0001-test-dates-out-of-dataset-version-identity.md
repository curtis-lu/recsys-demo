...
- 前處理的編碼是**純逐列** map lookup（`preprocessing.py:68-92`），查的是只在 train 上 fit 的 `category_mappings`；整個 `apply_preprocessor_to_features` 唯一的聚合是未知值計數，只餵給 `logger`，不進輸出。因此 test 日期不改變任何既有產物的**內容**。
