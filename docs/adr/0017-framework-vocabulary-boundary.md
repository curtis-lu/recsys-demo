...
> **2026-09-09 部分翻案（#328）：現在兩道都擋。** 標題後半已不是現況，詳見本節末尾。

`core/schema.validate_schema_config`（每個指令進入點都跑）在缺這三個角色時 raise；
~~`get_schema` 的內建預設本輪不動。~~ **（#328 已拿掉）**
...
### 2026-09-09 部分翻案（#328）：這個決定只剩上半句成立

標題「不擋 `get_schema`」現在**不再是現況**。#328 把三個角色從 `_DEFAULTS` 拿掉之後，
`get_schema` 缺這三個角色也會 raise——不是因為改了主意，而是因為沒有預設可回：
下一行的 `schema["entity"]` 本來就會 `KeyError`，差別只在拋出來的是一句看得懂的話
還是一個 `KeyError`。兩道閘現在共用 `core/schema._missing_roles_message`。

**仍然成立的是理由本身**：`validate_schema_config` 是**第一道也是完整的一道**——它在
CLI 進入點跑、在 Spark session 存在之前、一次收集全部缺項，所以真實使用者拿到的
訊息是幾秒內的完整清單，而不是冷啟動之後從某個節點深處炸出來的第一個缺項。
`get_schema` 那一道守的是**沒走 CLI 的呼叫端**，實務上就是測試——留著預設的話，
一個沒宣告 schema 的測試會拿到示例部署的欄名而照樣全綠（#274）。

上表那個 304 vs 48 的爆炸半徑量測仍然是真的，也仍然是「為什麼分兩張 PR」的理由；
它從來不是「永遠不要擋在 `get_schema`」的理由。R6 那一筆登記已隨 #328 移除。
...
- **報表 prose 與詞彙表裡的業務詞。** 只有**表格欄名標籤**解凍改成從 schema 取；
  報表其餘內容調整當時仍凍結。（2026-09-13 更正：該凍結已依 [ADR-0020](0020-evaluation-bug-round-intended-behaviours.md) 解除，地雷圖那一條已刪；這一點的「刻意沒做」仍成立，只是理由變成「不在那一輪範圍」。地雷圖檔案本身已於 2026-09-20 整份刪除，見 `docs/agents/issue-tracker.md`。）
