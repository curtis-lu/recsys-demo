...
**實作時補的一條（2026-09-13，#343，使用者同意併入）**：raise 只擋「視窗完全空」。審查指出另一個形態：`lookback_months: 12`、`label_table` 在視窗內只有 2 個月，不會 raise，報表卻照印「過去 12 個月」，「平均每月」也除以 12、低估 6 倍。這條**揭露、不擋**：`build_baseline_section` 用 `monthly_counts` 數出視窗內實際有資料的月份，少於 `lookback_months` 時句子加註「label_table 在這個視窗內實際只涵蓋 N 個月」，「平均每月」改除以實際月數。不擋的理由同本條的精神反過來用：部分歷史仍是真的歷史，不是答案；擋下會讓第一次上線（歷史還不滿 12 個月）整份月報跑不動。
...
- 「母體表有沒有這一欄」是**執行期的 metadata 讀取**（`spark.table(...).columns`，只碰 metastore、不掃資料），**由 `prepare_eval_data` 做一次**——它是唯一知道這次是哪個模式、也是唯一做 segment join 的地方。它把「這次實際 join 進來的 segment 欄」寫成一份小 JSON（新 catalog 條目 `evaluation_segment_columns`，`data/evaluation/${model_version}/${snap_date}/segment_columns.json`，是 `prepare_eval_data` 的第二個 output）。需要分群的消費者（`compute_metrics`、`compute_report_aggregates`、`draw_diagnosis_sample_node`、`restrict_to_common`）讀那份 JSON 決定要對哪些欄分群，**不看 frame 的欄位、也不各自去查母體表**。**不 raise**，缺欄就走下面「母體表沒有這欄」那條路。
  ...
  （2026-09-13 實作更正，#344：上面列的四個消費者有兩個寫錯。`compute_report_aggregates` 只選 item／score／rank／label 四欄，從不分群；`restrict_to_common` 也不分群。實際分群、因此讀這份 JSON 的是 `compute_metrics`、`compute_baseline_metrics`（baseline 的 per_segment）、`draw_diagnosis_sample_node`、`generate_comparison_report`（Model A 那側，`--compare-only` 讀的就是它）。另外兩點：這份 JSON 照〈橫切決定二〉寫進指紋，但**沒有 node 比對它**——標準 run 裡消費者都從同一次 run 拿到它（`eval_predictions` 只在記憶體，任何切片走到消費者都會重跑 `prepare_eval_data`），`--compare-only` 上 `--post-training` 是啞的，比對會拒絕每一個 post-training 寫的分區。前一個理由（「只在記憶體」）在 ADR-0018 決定 1 物化 `eval_predictions` 之後就不成立，屆時要回頭決定由誰比對這份指紋；A10 除了「覆寫完整」，也保留舊 A10 本來就有的「`segment_column` 等於欄名」，並多擋「鍵不在 `segment_columns`」的覆寫——覆寫現在用欄名查，舊寫法（鍵取別名、靠 `segment_column` 對應）不擋的話會通過檢查、從不被 join，那一欄悄悄改從母體表取。報表與 WARN 印的母體表名是 catalog 條目名（node 只拿到 DataFrame），不是上文範例的 Hive 全名；覆寫表印設定的 `table`。報表把 `sources`／`missing` 印在「基本統計」段，經 `compute_metrics` 帶進 `evaluation_metrics`，`generate_report` 的簽章不動。）
...
**實作時補的一條（2026-09-13，#343，使用者同意併入）**：「只過濾 `display.*_k` 清單」漏了一種表。比較報表的 overall 與大類 overall 表不讀顯示清單，而是把算過的鍵全部攤開（key-agnostic），3 個大類照樣印 `precision@4`／`@5`——正是稽核舉的那個情境。所以同一條規則（K 大於該粒度 item 數就不印，item 數 ≤ 0 時不過濾）也套到這種攤鍵的表，規則住在 `report_builder.py` 同一處，顯示清單與攤鍵兩條路共用。計算層仍不動。
...
- `prepare_eval_data` 寫出的每一列多一個框架自有的欄 `eval_partition_fingerprint`，值是 `PARTITION_CONTENT_KEYS` **扣掉 `evaluation.snap_date`** 的 sha256（`pipelines/evaluation/steps/config_fingerprint.py::partition_fingerprint`）。扣掉的理由：一個日期分區的內容只取決於模式與分群設定，不取決於同一次還評估了哪些別的日期；不扣的話，設定相同的單月執行與區間執行會互相擋。
- 每個讀 `enriched_eval_predictions` 的 node 改經 `steps/snap_date_scope.py::restrict_to_current_eval_partitions`：篩日期、確認每個日期的分區指紋、把這欄丟掉再往下用。指紋不同或是 NULL（#374 之前寫的分區，表的 schema 演化補 NULL）就 raise，一次列出所有有問題的日期，建議 `--from-node prepare_eval_data`。`tests/test_pipelines/test_evaluation/test_pipeline.py` 的 AST 測試改成要求讀者呼叫它。
- 比對的基準：標準模式的讀者比「今天的設定」。`--compare-only` 的兩個 A 側讀者（`validate_enriched_eval_predictions_present`、`restrict_to_common`）改比「這個目錄的 `segment_columns.json` 記錄的設定」（`recorded_partition_fingerprint`），因為那條路上 `--post-training` 是啞的，拿今天的值去比會擋掉每一個 post-training 寫的分區——正是上面〈不比的地方〉不比的理由。兩個 node 因此多收 `evaluation_segment_columns` 這個輸入。`--compare` 模式下那份 JSON 是同一次執行剛寫的，兩種基準一致。
- 成本：單一日期時對那個分區 `limit(1)` 取一列（一個分區由一次 dynamic overwrite 寫成，整格同一個指紋，所以一列就夠）；多個日期時一次 `select(日期, 指紋).distinct().collect()`，不是每個日期一個 job。缺資料（某日期零列）由同一個 job 回報，交給呼叫端處理（`compute_metrics` 的後置檢查、`--compare-only` 的閘門）。
- B 側（比較對象的 enriched 表）不驗，只把這欄丟掉：它帶的是 B 自己那次執行的設定。
- `_require_prepared_with_current_config`（這份 JSON 對今天設定的比對）保留：分群清單仍然從它來。
- 上面〈接受的漏洞〉裡「`compute_report_aggregates` 與 `restrict_to_common` 讀表但不讀這份 JSON」那一條，因為它們現在比分區自己的指紋而關上；「Runner 先存分區、再存 JSON 之間中斷」那一條，分區這一半也由分區指紋擋下。
