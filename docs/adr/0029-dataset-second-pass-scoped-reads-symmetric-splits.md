...
> - **更正決定 8 的行數估計**：`nodes.py` 在本份寫成時（`2e99c573`）是 1733 行，本票開工時（`6d1d8ca5`）是 1999 行，本票做完（merge 進 main 的 `2ec9b792`）是 1807 行，不是 1200–1300 行。估計錯在方向：決定 4 沒有刪掉 node，是把 val、test 在 model_input 上丟組的兩個 node 換成在 keys 上丟組的兩個，同時讓 B10 多比 val、test 兩對，所以 #461 讓它多了 159 行（#460 多 66 行、#462 多 41 行）。決定 8 不拆檔的理由不靠行數，結論不變。
...
> - **更正上文「兩張表都要有分區」**：`dataset.train_dev_ratio` 是 0 時，只看 `train_model_input`。這個設定是支援的，它故意讓 train_dev 是空的：`split_train_keys` 只在 ratio 不是 0 時擋空的 dev，B10 也放行沒有檔案的 split。而空的 frame 寫不出分區。照上文照做的話，這種部署每次完整執行都成功，`latest` 卻永遠不會建起來，警告還叫人去跑剛跑完的東西。其他值（含沒寫）照樣要求 train_dev。這條由審查的「本份哪裡寫錯」視角查出。
> - **已知的落差，照原樣留著**：開跑前只查 `train_model_input`（照上文），跑完之後查兩張。所以一個只建了 `train_model_input` 的 variant（例如切片只跑了 `build_train_model_input`）會通過 A55，跑完 test 鏈之後由 B10 或跑完之後的警告大聲擋下，不會靜默；只是擋得晚，訊息也沒說是缺 train_dev。上文「開跑前的檢查與跑完後的 `latest` 規則用的是同一個證據」要照這條讀。
> - **本決定不處理的一件（既有行為）**：「表在」不等於「表通過了檢查閘」。上一輪在 B10 被擋、非零退出之後，再跑一段不含 B10 的切片，那個 variant 的表都在，會被發布。#463 以前也一樣會發布；`--only-test-months` 含 B10，會再擋一次。
...
> - **過期行號只剩 4 處**，不是 6 處：規則 5 那兩處（`:202-`、`:383-`）在 #462（`4e9aab66`）改寫那一節時已經改成函式名。
...
>   - ADR 裡的行號改成函式名：第 7 列（ADR-0001、0002、0011）。只換定位，不改內容，先例是 `34faaac2` 對 ADR-0001、0003 的同類修正。ADR-0002 那段標了核對日期，另外註明了改動日期。
...
> - **審查後改的幾件**（三路審查，其中一路專找「本份與 #467 哪裡寫錯」）：
>   - node 規則 11 的表第二列補上「本 pipeline 剛落地的產物的 parquet 檔尾」。B8、B10 讀的就是這個（ADR-0006 的修訂），原本的表只寫來源表，規則 17 引用它時對不上。
>   - 規則 17 的範圍改成只用「判斷寫在 `core/consistency.py` 的 B 系列」界定，不再引用規則 11 的列。node body 要寫的事改成開放的：查哪些表、月份、欄、用什麼單位比，都是決策，清單以外的決策也留在 node（例如粒度閘的 0＝0 不算失敗）。照原本的封閉清單做，這些決策會被搬進 `steps/`，跟決定 7 實作註記警告的是同一種錯。
>   - `build_model_input` 那筆例外也違反規則 4（換成機械名之後，build 的 node body 只剩月份），補進違反的條目。例外只登記這一組；原本寫的「三個條件成立就算例外」刪掉，因為那等於不問使用者就能新增例外。審查另外建議把它改寫進規則 5、從例外表拿掉，那是放寬規則 5，留給使用者決定。
>   - 規則 18 原本寫「一批改動只加一次」，agent 看不到「一批」的邊界。改成：比對最新的發布 tag 裡常數的值，main 上已經比它大就不必再加；維護分支（`release/*`）上的修正要取 main 沒用過的值，否則兩邊加到同一個數。
...
#467 收尾時另外加的修訂（盤點 note 第三節的脫節）：0006、0021（`filter_groups_with_positives` 已不存在）、0008（node 數與 `steps/` 模組、evaluation 已重整）、0012（節點數）、0013（#281 加進清單的精度閘）。0001、0002、0011 只把行號換成函式名。
