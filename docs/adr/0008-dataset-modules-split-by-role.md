...
> **要照著做，讀別份；要知道當初為什麼這樣決定，讀這份。**
>
> - **第二、三節的判準已一般化成跨 pipeline 的判準**，真實來源改為
>   [`docs/agents/pipeline-node-design.md`](../agents/pipeline-node-design.md)（#197 把同一組判準
>   套到 inference 時搬出去的）。兩者衝突時以那份為準。
> - **實作狀態（2026-08-19 核對）**：下方「後果」列的四張票**已全部落地**——#166（A24）、
>   #167（CLI 走 catalog）、#168（`preprocessing/` 解散）、#170（決策上浮；票 4 拆成 T4a／T4b，
>   #170 是後者）。第一節盤點的五個檔案今日皆已不存在。
> - **讀法**：第一節與第二節後半引用的行號是 **2026-08-05 決策當時**的程式碼，用來指認被改掉的
>   舊形狀；指向現況的引用已按今日核對過，並標明。
...
用當時的現況驗這條線：`select_keys` 一個呼叫裡有四個決策（月份過濾／有效抽樣率的覆寫優先序／
...
`month_plans.py` 留在根層的判準是**它有 src 側的外部消費者**：`__main__.py` 在 pipeline
...
`month_plans.py` **必須維持零 pyspark import**：它與 `nodes_shared.py` 當時的 451 行測試
...
61 行真正共用的機制（類別編碼、float32 轉型）留在單一模組 `preprocessing.py`；
`apply_preprocessor` 併進 inference 既有的 wrapper；feature-selection 那 38 行進 `models/`
（`models/lightgbm_adapter.py:38` 也用它，放 `pipelines/training/` 會製造 `models/` →
`pipelines/` 的反向依賴，而 `models/`／`utils/`／`core/` 目前對 `pipelines/` 是零 import）；
其餘全部是 dataset 專用，進 `pipelines/dataset/`。

**`preprocessor.json` 的四鍵契約也留在 `preprocessing.py`。** 解散之後 writer 進
`pipelines/dataset/nodes.py`、reader 進 inference 側，而
`feature_columns`／`categorical_columns`／`category_mappings`／`drop_columns` 這組鍵是**真正
跨 pipeline 的東西**——當時它只靠 writer 與 reader 剛好同檔而可見（`_spark.py:214-219` 寫、
`:411-414` 讀），解散後就沒有定義點了。失效模式是 dataset 側改鍵名或加鍵，inference 側到公司
環境才爆（inference e2e 本機難驗，撞當時未解的 #63），或更糟——`dict` 讀取靜默套用舊語意。
用 TypedDict／dataclass，至少 docstring 化。這不改形狀，只是讓那 61 行的模組多一個名正言順的
職責：它本來就該是「fit 端與 apply 端共享語意」的家。

### `__main__.py` 改走 catalog，以及一個會靜默全量重建的順序約束

當時 `__main__.py:217-243` 的 `_collect_existing_snap_dates` 直接讀 catalog 的**原始設定 dict**
（`entry.get("type") != "HiveTableDataset"`、`entry["database"]`、`entry["table"]`），自己知道
HiveTableDataset 長什麼樣、怎麼列分區。三個增量表都有
`partition_filter: {base_dataset_version: ...}`，所以 `HiveTableDataset.existing_partition_values()`
回傳的就是已按當前版本篩過的答案。`helpers_spark.py:24` 的 `existing_snap_date_partitions` 刪除
——它是同一件事的第二份實作。`__HIVE_DEFAULT_PARTITION__` 的丟棄守衛不能跟著消失，它是個
決策（丟棄＝該月視為未落地、會被重做），移進 `month_plans.py`。

**這個改動有一個順序約束，照字面實作會靜默全量重建**：

1. 當時 `__main__.py:682` 取得 `catalog_config` 時傳的 `runtime_params=params` **還不含**
   `base_dataset_version`——它在 `:692` 才算出、`:719` 才進 runtime_params。
2. 而 `core/config.py` 的替換是字串 `.replace()`，**未解析的 placeholder 靜默留成字面
   `${base_dataset_version}`、不 raise**。
3. 若拿那份 config 建 dataset 物件去問 `existing_partition_values()`，它會用那個字面字串逐一
   比對 partition spec、**丟掉全部**，回傳 `[]` → 每個月都視為未落地 → 全量重建，而且不報錯。

當時的實作把 `base_v` 當函式參數顯式傳進去，正是繞開了這件事。所以新實作必須在 `base_v` 算出
之後，用**含 `base_dataset_version` 的 substitution params** 重建（或延後建立）catalog 物件才問
它。這個坑不會被票 2 的其他驗收條件抓到——`conf/` diff 是空的、測試是綠的、DAG 沒變，只有
`[months]` log 會顯示 processed=全部而沒有人斷言它。驗收要另加一條：**對已落地的表實測
`plan.skipped` 非空**。
...
| 舊名 | 新名 | 為什麼 |
|---|---|---|
| `_get_preprocessing_config` | `prepare_model_input_config` | **不只去底線**：舊名指向一個不存在的設定鍵。它讀的是 `dataset.prepare_model_input`（`steps/feature_columns.py`，區域變數就叫 `pmi_config`），config 裡沒有 `preprocessing` 這個鍵 |
...
> 為何都靜默，**不是呼叫點的分界線**。`feature_table` 今天同時出現在兩側，而
...
legend 已經有「runtime backstop」這個登記過的模式（A2 的 item guard、A3 的 identity-cat
guard、B6 在 `io/extract.py` 的第二個掛點），所以「node 裡有 raise」本來就不等於違規——
**違規的是沒登記**。

照這條規則重掃，唯一無家可歸的是 `validate_date_splits`（當時的 `nodes_shared.py:11`）：它是純
config 判斷（四個 split 的日期互斥），legend 零登記，卻藏在 `select_train_keys` 裡當副作用。
...
CLI 指令的 entry 都跑，而只有 dataset 讀這些鍵。A21／A22 也是因為「aggregator 看不到需要的
context」而各自接在指令上。
...
當時 `_spark.py` 的三筆（`:152` 缺 train 月份、`:266` 缺 base-key 欄、`:281` 缺 to_process 月份）
...
A24 落地時順帶把比對正規化：當時的 `validate_date_splits` 用 `str(d)` 比字串，
`"2026-01-31"` 與 `"2026-1-31"` 這種同日不同寫法抓不到。改成先轉 `pd.Timestamp` 再比。
**這是刻意的行為收緊，不是純搬移**——原本放行的設定可能開始 raise，屬於票 1 的預期效果，
不進版本 hash。

## 四、可機械檢查的，與不可檢查的

新增兩條結構約束，登記在 `docs/agents/architecture-constraints.md` 節二（今日在該檔的
`### S1`／`### S2`）：
...
（見下方「考慮過但否決的選項」與 ADR-0002 那個三天就過時的補釘），同一把尺必須量到自己。
成本趨近零：`tests/test_core/test_architecture_constraints.py` 已有 AST 稽核基建，加一條
import 掃描沒有假陽性面。

### 2026-08-07：`steps/` 子套件在 S2 上開了一個必須同時修掉的洞

該檢查沿 `pipelines/dataset/` 的同層 import 遞迴，把模組名解析成「同層目錄 ／ 名字 `.py`」。
搬進子套件之後，`recsys_tfb.pipelines.dataset.steps.scoping` 這種帶點的路徑會被解析成
`dataset/steps.py`——那個檔不存在，遞迴回傳「查無」，於是**檢查靜默放行**。

而失效方向正是它要守的那一個：`month_plans.py` 哪天 import 了 `steps/` 裡任何 Spark-typed
模組，兩條檢查都不會紅。所以解析必須按 `.` 展開成子路徑，並補一個「解析器真的跨得進子套件」
的測試——否則 S2 的第二條會退化成結構上不可能紅的裝飾品，而搬檔這個動作本身就是製造它的
原因。

### 為什麼是「必須 def 定義」、為什麼前綴用 S
...
`core/consistency.py` 的 A 系列**已經在撞車**（兩邊都有 A5、A7，意思完全不同）。本次不
...
可行性範本。代價：短期內 A1 的檔名代理與 S1 的 def 比對並存。（#163 至今仍開著。）

**S1 只擋位置，擋不住內容。** 一個 12 行的轉手 node 加一個四決策的 helper 完全滿足 S1
——那正是當時 `select_train_keys` → `select_keys` 的形狀。所以第二節的判定程序必須被
...
- **對齊**：票 2 把儲存知識收回 catalog，就是 C-3（「檔案路徑與儲存邏輯不得散落在
  codebase 各處」）。當時 `__main__.py` 自己知道 HiveTableDataset 有 `database`／`table`
  欄、知道怎麼列分區，正是那句要擋的。
...
- **不受本次影響的既有偏離**：本框架的正確性重心在集中式 predicate（`core/consistency.py`）
  而非 node 契約（`core/node.py`），見 `architecture-constraints.md` F7。本 ADR **沒有**把
  重心搬回 node——第三節的歸屬規則反而是強化 predicate 那一側。
- **沒有把 kedro 明文不要求的東西當成要求**：§2.3／§2.4 列出 kedro 對 node 寫檔、log、
  行數都沒有規定。本 ADR 也沒有據此立規則。

## 考慮過但否決的選項

**node 保持薄轉手，靠 docstring 說明 ML 決策。** 當時的轉手 node 就是這個做法的樣板實作
（`nodes_spark.py:237` 用 14 行 docstring 講清楚 [ADR-0002](0002-preprocessed-feature-table-incremental.md)
為何跳過既有月份是安全的），品質不差。否決的理由不是它做得不好，是**散文描述「邏輯住在哪」
會過時而沒有任何機制逼人發現**：ADR-0002 就有一段補釘——原文寫的「差集邏輯集中在
`nodes_shared.py` 的單一 helper，由四個 node 共用」在三天內被
[ADR-0007](0007-month-plans-travel-through-the-catalog.md) 改掉，散文留在原地。用可執行的呼叫
描述決策，改了 diff 就在同一處。
...
把它套到什麼上（inference 的 pipeline 把 `preprocessor` 當 catalog input 收下，四個值全從已
落地的產物讀，不碰 `parameters`）。真正被兩邊共用的只有 61 行編碼機制。保留會讓它繼續裝
400+ 行單一消費者的碼——正是 `nodes_shared.py` 那個病。
...
`select_keys` 當時的形狀就合規，這條判準就擋不住它要擋的東西。
...
左右（估計值，非量測），落在 repo 常態內，但單一 node 函式會明顯超過
`architecture-constraints.md` F8 記錄的「七成在 40 行內」。**不要把它「修好」。**
（2026-08-19 實測：`pipelines/dataset/nodes.py` 684 行，估得偏低；同期 `training/nodes.py`
1443、`inference/nodes.py` 717、`evaluation/nodes_spark.py` 617。）
...
而別人有時，那不是 dataset 的例外，是別人還沒改。（inference 側後來由 #197／#188 補上，
今日只剩 `evaluation/nodes_spark.py`。）

**切成四張票，順序有理由：**

| 票 | 內容 | 依賴 | 實作票 |
|---|---|---|---|
| **1** | `validate_date_splits` → A24，接在 `dataset` 指令上 | 無 | #166 |
| **2** | `__main__.py` 改走 catalog、刪 `existing_snap_date_partitions` | 無 | #167 |
| **3** | `preprocessing/` 解散。純機械搬移，不改形狀 | 無 | #168 |
| **4** | dataset 內部重整成上面的檔案清單 ＋ S1／S2 | 票 3 | #170（T4b） |

票 1 **先做**，因為它最小、零依賴，且它決定了第三節那條歸屬規則到底怎麼落地——後面三張票
都在同一份 legend 與同一個指令入口上疊東西，這條先釘好，後面不必回頭改。（初版把它排第一的
理由是「唯一會集體弄紅測試」；改成接在 dataset 指令上之後那個風險消失了，見第三節。）

票 4 依賴票 3（碼要先搬進來才能上浮決策），也是唯一需要判斷力的一張。不再細切：票 4 內部是
同一個判準的重複套用，切成三張只會多兩輪 fresh-context 審查、不降低風險。
...
- **同源的 `_spark` 遺跡**：~~`evaluation/` 與 `inference/` 的 node 不照第二節的判準重寫~~
  **inference 側已由 #197 補上**（並在 #188 一併改名為 `nodes.py` ＋ `steps/`）；
  **今日只剩 `evaluation/nodes_spark.py`**。
...
- **兩套 A 系列編號的撞車**：`architecture-constraints.md` 的 A1–A7 與 `core/consistency.py`
  的 A 系列。本次只加一句說明，不重編號——重編號會讓既有文件的引用全部指錯，理由同
  A16/A17/A18 退休不回填。（今日仍然如此。）
- **inference 無法自我檢查「載入的 preprocessor 是否與現行 config 相符」**：`compute_feature_columns`
  ...
  能力——inference 的把關本來就是 artifact 對 artifact，不是 artifact 對 config。
  [ADR-0011](0011-inference-validation-two-layers.md) §5 把這項殘留縮小成「模型的
  `feature_names()` 必須是 `preprocessor.json` 的保序子序列」，但**沒有消掉**。
...
- **`recsys_tfb.preprocessing` 那兩個底線函式**（`_cast_feature_floats_to_float32`、
  ...
  `pipelines/dataset/` 內，而該模組的改動會同時碰到 inference。（今日仍是底線名，
  `preprocessing.py:115`／`:179`。）
- **`feature_table` 同時被正規化與不正規化的月份過濾**：兩個呼叫點對同一個 catalog 條目做了
  不同選擇。這個不一致早於本次改動，rename 不改行為也就沒有動它；第二節的命名論證已更正為
  「正規化與否是逐呼叫點的選擇」，但**哪一邊才對還沒有人判定**。（今日仍在：`nodes.py:480`
  不正規化、`:574` 正規化；前者後來補了理由註解「來源表的時間欄是真 DATE」，後者沒有。）
...
- `_cast_feature_floats_to_float32` → `cast_feature_floats_to_float32`、
  ...
  （AST 逐檔比對：去掉 docstring 後兩版的 `ast.dump` 完全相同）。
...
- 改到的檔案共 13 個：src 5（`preprocessing.py`、兩條 pipeline 的 `nodes.py`、
  `io/extract.py`、`pipelines/dataset/steps/sampling.py`）＋ tests 1
  （`tests/test_preprocessing.py`）＋ docs 7（上列三處登記、`architecture-constraints.md`、
  `docs/pipelines/inference.md`、`docs/operations/known-pitfalls.md`，以及 2026-07 OOM
  調查紀錄的檔頭識別字註記——**該文正文一律不改**，比照 #199 對同一份文件的處置）。
- **只有 4 個檔改錯會讓測試轉紅**（`preprocessing.py` 的兩個 `def`、兩條 pipeline 的
  import ／呼叫點、`tests/test_preprocessing.py`）。其餘 9 個都只在散文裡提到舊名，
  **改錯測試全綠**，只會留下指向不存在識別字的假引用——所以這張票靠 grep 白名單驗收，
  不靠 CI。
...
| 檔案 | 為什麼不改 |
|---|---|
| `docs/adr/0001`、`0002`、`0008`（本節原文）、`0010` | ADR 原文記錄的是當時的事實，一律只加標日期的修訂 |
