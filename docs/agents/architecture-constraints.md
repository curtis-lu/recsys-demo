...
`tests/test_core/test_architecture_constraints.py` 執行，**39 個測試，3.08–3.15 秒**（2026-09-09 連跑三次，`load average 2.5`；S6 加進來之後——它只掃 `src/` 的 142 個檔，但那是本檔第三次各自完整解析同一批檔案）。⚠ **次秒級的數字本來就抖，別當精確值引用**——同一台機器上，本檔前幾版分別量到 26 個測試 1.06–1.10 秒、20 個測試 1.4–1.7 秒、17 個測試 1.25 秒，而更早的版本寫 0.62 秒。要引用就自己重跑一次。
