...
**在等什麼**：出現要讀比較數字的人，或量到重算是瓶頸。
**什麼時候可以刪**：兩者任一出現、決定做或不做之後。
...
**在等什麼**：使用者在「搬進 registry」與「留原位、只換設施」之間做決定，或決定都不做。
**什麼時候可以刪**：決定之後。

### 別順手刪掉 evaluation 的 `calibration_bins` 和它的兩個設定鍵

**這在講什麼**：`compute_report_aggregates` 會照 `include_calibration`、`n_calibration_bins` 算 calibration bins，寫進 `report_aggregates.json`；但主報表刻意不畫它（這個框架的目標是排序，score 不是機率）。看起來是白付 Spark 的錢，刪掉很乾淨。
**為什麼現在不做**：刪不刪沒被裁決過。它跟上一條（兩套機制要不要統一）是稽核拆開的兩件事，小的這件不該趁大的那件做不動時先偷做。真要刪，`build_item_detail_section` 的 docstring 裡「calibration 曲線移到獨立診斷報表」那句要一起改，因為那份獨立報表並不存在。
**證據**：`docs/notes/2026-09-09-evaluation-audit.md` 的設計問題 B（「動它之前要知道的事」第一點）。
**在等什麼**：使用者決定刪不刪。
**什麼時候可以刪**：決定之後。
...
**在等什麼**：真的要加新的指標家族，或使用者決定開這一輪。
**什麼時候可以刪**：兩者任一發生、那一輪做完或決定不做之後。
