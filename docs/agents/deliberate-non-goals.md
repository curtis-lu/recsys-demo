...
### 別給日期區間加「最近 N 天」這種相對寫法

**這在講什麼**：日期設定可以寫成 `{start, end, step}` 區間（#374，`core/date_ranges.py`）。很自然會想再加一種「從執行當天往回 N 天」的寫法，省得每次改起訖日。
**為什麼**：版本 ID 是拿展開後的日期算的。相對寫法讓設定檔一個字沒改、版本卻每天換：dataset 天天整批重建、模型天天重訓，事後也重現不了當時的結果。使用者 2026-09-18 拍板不做（#374 的範圍外項目）。`core/date_ranges.py` 的模組 docstring 與 `docs/pipelines/dataset.md` §3.1 寫著同一個理由。
**什麼時候可以刪**：版本 ID 改成不由展開後的日期決定，或使用者改變主意。

### 別放寬架構稽核 A1 的掃描範圍，也別因為它有盲區就把檢查拿掉

**這在講什麼**：A1 的兩個檢查（catalog 存取、直接寫檔）用**檔名**挑要掃的檔，只讀 `pipelines/**/nodes*.py`。所以 `steps/` 底下的程式碼、還有 `def` 寫在別的檔的 node，都看不到。盲區清單與重盤方法在 [`architecture-constraints.md`](architecture-constraints.md) A1 的「這個檢查看不到」。看起來改個 glob 就好。
**為什麼**：使用者 2026-09-19 拍板（#163）：盲區只記進文件，不放寬掃描、不把 `diagnosis/model/` 那兩個自己存圖的 node 補進 R4、也不改它們。理由：
- 看不到的地方裡，真的有寫檔的只有 `diagnosis/model/`，而使用者傾向大幅簡化診斷，現在補可能白做。
- 放寬不是換個 glob 就好：當天實測會多掃到兩個不是違例的檔，要先處理（是哪兩個，見上面連過去的那一節）。
- 檢查本身留著：跑不到一秒，而且真的影響過設計——`pipelines/training/nodes.py` 的 5 個 cache node 刻意把刪檔留在 node 裡，就是為了讓它看得到（R4 表下的注記）。

**也別做**：靠改檔名讓稽核看得到，或新增一條讓自己合規的規則。
**什麼時候可以刪**：`diagnosis/model/` 的診斷簡化有了定案（那些 node 拿掉，或決定保留），或使用者重開這件事。
