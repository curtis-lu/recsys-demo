...
**現況**：issue #163，狀態查 `gh`。稽核本身在 `tests/test_core/test_architecture_constraints.py`，搜 `rglob("nodes*.py")`。dataset 那一帶後來改由另一條規則守住，所以 #163 是被**繞開**而不是解決。對 dataset 以外仍然失準（例：training 有 node `def` 在 `recsys_tfb/diagnosis/model/` 底下，不在 `pipelines/` 裡，這個掃描看不到）。
