...
改變的是另一半。2026-09-16 設計第二種使用情境（線上廣告推薦的離線訓練與評估，見 ADR-0021 起的那一批）時，`time`、`entity`、`item`、query group 這類跨模組的詞沒有一個地方集中定義，意思散在各 pipeline 文件與錯誤訊息裡；討論中同一個詞被用成不同意思（例如 `time` 被當成「到秒的曝光時間」，其實框架裡它是分組與切分用的時段）。這類詞的定義不是實作，放進 `CONTEXT.md` 沒有「只能寫近似」的問題。
