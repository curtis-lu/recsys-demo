...
**開跑前它會先確認一件事**：這個模式不建訓練資料，只能沿用以前建好的那一份。所以它會先向 metastore 問：目前設定算出的 `base_dataset_version`、`train_variant_id` 底下，`train_model_input` 有沒有分區。沒有就在任何 node 執行之前停下來，訊息帶 `(A55)`，而且什麼都不寫。訊息有兩種：
...
以前這兩種情況都不會被擋：這一輪照樣跑完，步驟 3 讀到 0 列也不報錯。
...
**跑完之後**，`data/dataset/<base_dataset_version>/train_variants/latest` 會指向目前設定的那個 `train_variant_id`（能走到這一步，就代表它建過）。步驟 3 讀的就是它。
