...
- **version** 是 `pyproject.toml` 裡的字串，發布時與 tag 名稱對齊：tag `v0.3.0` 對應 `version = "0.3.0"`。checkout 之後想確認自己手上是哪一版，看這個欄位。
...
`git checkout <tag>` 之後 git 會說你在 detached HEAD。這是正常的，代表你停在一個固定的快照上而不是某條 branch 的最前端。要在上面改東西就自己開一條 branch：`git switch -c my-deployment v0.3.0`。

執行環境不能連網時，在能連網的機器上打包該 tag 再搬進去：

```bash
git archive --prefix=recsys_tfb-v0.3.0/ --format=tar.gz -o recsys_tfb-v0.3.0.tar.gz v0.3.0
sha256sum recsys_tfb-v0.3.0.tar.gz     # macOS 沒有這支指令，改用 shasum -a 256
```
...
git fetch --tags
git tag                                        # 有哪些新版本
git diff v0.2.0..v0.3.0 -- conf/base/          # 預設設定改了什麼
git diff v0.2.0..v0.3.0 -- conf/sql/           # 來源表 SQL 範本改了什麼
git diff v0.2.0..v0.3.0 -- conf/spark-local/   # Spark 設定範本改了什麼
git checkout v0.3.0
...
```bash
git diff v0.2.0..v0.3.0 -- conf/ > upgrade-v0.2.0-to-v0.3.0.diff   # 能連網的機器上
diff -ru recsys_tfb-v0.2.0/conf recsys_tfb-v0.3.0/conf              # 或兩版都解壓後比對
```
