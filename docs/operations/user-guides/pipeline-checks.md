...
- **evaluation**
  ...
  - `evaluation.report.sections` 有寫任何開關的話，開關名稱必須剛好是 `dataset_overview`、`primary_map`、`diagnostics`、`baseline`、`diagnosis_links` 這五個，多一個或少一個都擋（`A34`）。
...
- **跑之前**（會擋，Spark 還沒啟動）
  ...
  - `--var` 與 YAML `variables` 的檢查（`A35`）：`--var` 帶到沒在 `variables` 宣告的名字、同一個名字帶了兩次、`--var` 缺 `=`、`variables` 的值不是字串或 `~`、`variables` 本身不是「名字 → 值」的對照、某個 `~` 名字沒有對應的 `--var`；`target_date`、`target_db` 這兩個保留名不管從 `--var` 帶還是寫進 YAML `variables`，都擋。這些問題會一次全部列出。
