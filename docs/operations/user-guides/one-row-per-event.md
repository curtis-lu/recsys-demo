...
| 三項排序診斷（`config_shift`、`item_ability`、`suppression`） | **自動跳過**，報表的〈完整性檢查〉會寫出原因。它們假設同一組內 item 唯一（例如 `suppression` 把「負例排在正例之上」彙總成 item 對，同一個 item 會變成壓制自己）。`model_capacity` 照跑——它只讀模型本身。 |
