...
做法是替它在**現在的計分月份**上重算分數。模型不會被重訓，也不會產生新版本：用的是那個版本已經訓練好的模型，只補預測、重算 `evaluation_results.json`。

**先確認你屬於哪一種：**

- 這個版本就是**現在的設定**算出來的那一個（沒改過 `training:`、也沒換過 dataset 版本）→ 只要做步驟 3、4，而且不用帶兩個版本旗標。加了評估月份後想讓 promote 用新月份比，就是這種，見 [新增一個評估月份](adding-an-eval-month.md#要讓-promote-用新月份比)。
...
`Selection metric` 是現在的**選版指標**（promote 拿來排名的那個指標），`Scored months` 是現在的**計分月份**（promote 只比在這些月份上算的分數）。做完之後這兩行要一模一樣，這是「設定有改回來」的證據。
...
cp conf/base/parameters_training.yaml conf/base/parameters_dataset.yaml rescore-backup/
```

如果你平常是改 `conf/<你的環境>/` 底下的檔案，就備份那裡的，下面每一步也改那裡的。
...
2. 在 `test_metrics:` 底下把**選版指標寫死**成動手前 ① 抄下的那一個：

```yaml
test_metrics:
  selection_metric: macro_per_item_map   # 換成你在 ① 抄下的 Selection metric
```

**為什麼要寫死選版指標**：它沒寫的時候跟著 `training.hpo_objective` 走。你剛還原的 `training:` 帶著那個版本當初的 `hpo_objective`，可能跟現在的不一樣：

- 現在的尺是二元預測類時，重算出來的分數就沒有這把尺的值，白做一趟。（`mean_ap`、`macro_per_item_map` 兩個排序類指標每次一律會算，所以尺是排序類時不會少值。）
- 更麻煩的是忘了改回來：promote 之後會用舊的尺排名，畫面上只有 `Selection metric:` 那一行看得出來。
...
`training:` 沒有還原到一模一樣。常見原因：只改了部分鍵；或 `conf/<你的環境>/parameters_training.yaml` 也寫了 `training:` 底下的鍵，蓋過了 base 的值。修到印出 `<舊版本>` 為止，**不要往下做**——版本號不同，後面每一步都會對著另一個（可能還不存在的）版本做事。
...
這是最後一次建這個 dataset 版本時的 `test_snap_dates`。動手前 ① 的 `Scored months` 每個月都在裡面 → **跳過這一步**。有月份不在裡面 → 往下做。
...
`on` 後面要是動手前 ① 的 `Scored months`，`selection metric` 後面要是 ① 的 `Selection metric`。`HPO objective` 是那個版本當初的，跟現在不一樣是正常的。
...
```bash
cp rescore-backup/parameters_training.yaml rescore-backup/parameters_dataset.yaml conf/base/
diff rescore-backup/parameters_training.yaml conf/base/parameters_training.yaml
diff rescore-backup/parameters_dataset.yaml conf/base/parameters_dataset.yaml
```

兩個 `diff` 都沒有輸出，才刪掉 `rescore-backup/`。

**做過步驟 2 的話**，再用現在的設定跑一次 dataset，讓 `latest` 指回現在的版本：

```bash
python -m recsys_tfb dataset --env <你的環境>
readlink data/dataset/latest
```

log 的 `base_dataset_version:` 與 `readlink` 印出的路徑，都要是動手前 ③ 抄下的那個版本。現在的 dataset 版本早就建好了，這一輪每個月份都是 `skipped`，很快就跑完。
...
1. `<舊版本>` 列在 `Ranked` 底下。
2. `Selection metric:` 與 `Scored months:` 兩行跟動手前 ① 抄下的一模一樣。不一樣代表設定沒改回來：promote 正在用別把尺、別份考卷排名。
3. `readlink data/dataset/latest` 是動手前 ③ 抄下的那個版本。
...
這時候的選擇：讓大家都改用一把這個版本有值的尺（把 `test_metrics.selection_metric` 明寫成排序類指標），或把考卷縮回這個版本算過的月份（`test_metrics.snap_date`）；兩者都會改變**所有**版本的比較方式。要不然就是用現在的設定重新訓練。
...
| 步驟 4 的 `selection metric` 不是 ① 抄下的 | 步驟 1 沒寫死選版指標。補上後重跑步驟 4 |
| 報錯訊息帶 `A54:` | 選版指標是二元預測類，但那個版本的 test 算不出來。見〈補不回來的情況〉 |
| 做完之後 promote 的 `Selection metric:` 或 `Scored months:` 跟 ① 不一樣 | 設定沒改回來。重做步驟 5 的 `cp` 與 `diff` |
