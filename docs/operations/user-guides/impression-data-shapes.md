...
線上每一次排序是一次請求：同一刻，幾個候選被一起排出來，挑前面的展示。如果你的資料有一欄認得出「這幾列是同一次請求」（例如 `request_id`），而你要離線也只在同一次請求裡比名次，就宣告 `occasion`。如果沒有這一欄，或者你要把一個 entity 一整個時段的曝光放在一起比，就不宣告。
...
|---|---|---|
| **一組 ＝ entity × 時段**（不宣告 `occasion`） | 什麼都不宣告（框架預設） | 只宣告 `event`（下文叫**形狀一**） |
| **一組 ＝ 一次請求**（宣告 `occasion`） | 只宣告 `occasion`（下文叫**形狀二**） | 兩個都宣告 |
...
- 兩個都宣告，只在「同一次請求裡同一個 item 會出現兩次」時才需要。示例沒有這種資料。

## 兩種形狀各適合什麼、各自的代價

```
形狀一：一組 ＝ 一個 entity × 一個時段      形狀二：一組 ＝ 一次請求
...
**形狀一**的組比較大，每組有很多列可以比。代價是它把不同時刻的曝光放在一起比：週一早上的素材 A 和週五晚上的素材 C 搶同一個名次，而線上從來不會這樣比。另外，宣告了 `event` 卻沒有逐筆的即時特徵時，同一個素材的多列分數完全相同，名次由同分規則決定（見 [`one-row-per-event.md`](one-row-per-event.md)〈同分〉）。

**形狀二**最接近線上真正在做的事：組裡就是那一刻被一起排的候選。代價是組很小。廣告示例平均一次請求 4 個素材，5% 的請求只有 1 個。下一節是這件事對指標的影響。
...
所以在形狀二的資料上，**mAP 要跟報表裡的熱門度基準線比，不要跟 0 比**。
...
| `k_values: "all"` | 仍解析成 item 的種數。組內 item 不重複，所以沒有一組比 item 種數長，這個 K 不會截斷任何一組 |
...
| 比較報表拿 `source: ranked_predictions` 當對手 | **在 CLI 入口被擋下**（A11），理由同上。用 `training_eval_predictions` 或 `enriched_eval_predictions` |
...
3. **離線只重排被曝光過的候選，線上是從全部候選裡挑。** 形狀二的每一組是舊系統挑好的 1～6 個素材，離線只問模型「這幾個怎麼排」；線上它要從全部 12 個裡挑。排得好這幾個，不代表挑得好那 12 個——那些沒被展示的素材根本沒有 label 可以評。
...
- **`occasion` 的欄不得是 NULL。** 有 NULL 時，training 會把所有 NULL 的列併進同一組，評估則把它們當成另一個組，兩邊各算各的、沒有錯誤訊息。擋法是把 `occasion` 的欄列進 `sample_pool` 與 `label_table` 的 `primary_key`（見③）。
...
- **比較報表的另一邊要是同一種形狀跑出來的。** 拿宣告 `occasion` 之前訓練的模型版本來比，它的預測表沒有 `occasion` 的值，兩邊一組也對不上，報表會停在「common_query_groups is empty」。
