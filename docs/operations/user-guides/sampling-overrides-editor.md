...
#### ⚠️ 權重的**整體大小**也會影響模型，不只相對比例

兩張成比例的權重表**不是**同一張表。實測（合成資料、`lambdarank`）：整張表乘上一個常數
把平均權重從 0.046 拉到 1.0，macro per-item AP 從 0.2723 變成 0.3311——只改尺度、
相對關係一個都沒動。多訓 16 倍輪數也追不回來。

**代數說這不該發生**（`葉值 = −Σg/Σh`，分子分母同乘會約掉，而本專案 `lambda_l2 = 0`），
而且原因**還沒查出來**。調查紀錄、重現腳本、以及已經排除的八條假設都在
[`../../notes/2026-09-08-lambdarank-weight-scale-anomaly.md`](../../notes/2026-09-08-lambdarank-weight-scale-anomaly.md)。

實務上：本節的公式（`w_pos = A`、`w_neg = A·v`）因為 `A ≤ 1`、冷門 item 的 `v ≪ 1`，
**匯出的平均權重天生遠小於 1**。在查明之前，把整體尺度當成一個要搜的超參數，
不要假設「照公式算出來的絕對值就是對的」。
