...
| dataset 報 `(A24) dataset.X_snap_dates and dataset.Y_snap_dates both contain [...]`、還沒起 Spark | 步驟 1 加的月份已經在 train／calibration／val 裡了（不變量 A24） | 從不該擁有它的那一組移除。日期是**按日比對**的，所以 `2026-1-31` 與 `2026-01-31` 也算同一個月。這條之所以要擋：同一個月同時訓練又評估，該月的指標會靜默變成 in-sample 數字，報表看起來完全正常 |
