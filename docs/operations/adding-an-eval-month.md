...
2. **`train_snap_dates` 或 `calibration_snap_dates` 是空清單**（後者要配 `enable_calibration: true` 才有影響）。這是旗標前提唯一一個會失效的情況，而且目前**沒有任何不變量擋得住它**，所以寫在這裡：`select_sample_keys`（node 名；它跑的函式叫 `select_train_keys`）與 `select_calibration_keys` 走的是 `restrict_to_months_or_all`（`pipelines/dataset/steps/scoping.py`），月份清單為空時**不過濾、整池照收**。於是這兩個 node 的內容綁在**整張 `sample_pool`** 上，而它是上游 ETL 逐月長大的來源表。帶旗標安不安全，因此不再由你的 config 決定，而是由「上次跑完整 dataset 之後 ETL 有沒有再加月份」決定：加過，被跳過的 keys 就停在舊的；沒加過，其實沒事。**沒有任何東西在追蹤這件事，你從 log 也看不出來是哪一種。**（`select_val_keys` 走的是嚴格的 `restrict_to_months`，不受影響。）
