...
- **⚠ 2026-08-01 追加：這個症狀現在有兩個獨立成因，要依序排除。** dataset 也變成增量的了（[ADR-0002](../adr/0002-preprocessed-feature-table-incremental.md)）：既有月份的 partition 只要存在就會被跳過，所以「重跑 dataset」本身**不會**重算舊月份。先確認 dataset 那一層真的重算了（`python -m recsys_tfb dataset --env local --rebuild-dates <該月>`，log 應印 `[months] test branch: processed=<該月>`），再處理下面的 cache 層。兩層都要過，只修一層數字照樣不動。
- **規則**：重算既有月份之後，**先刪掉該月的 cache 目錄再跑 predict**：`rm -rf <cache.root>/<base_dataset_version>/test_months/<YYYYMMDD>`（本機 `cache.root` = `data/recsys_cache`）。單純**新增**月份不受此坑影響——新月份沒有目錄，必然 miss；而且若忘了先跑 dataset，逐月複製的 glob 零命中會 raise `FileNotFoundError`，是 fail-loud 而非靜默。
- **驗證方式**：`ls -d data/recsys_cache/<base_dataset_version>/test_months/*` 看目前 cache 了哪幾個月。跑 predict 時 log 對每個月份會印 `cache_hit name=test_model_input path=...` 或 `cache_miss ...`——你要重算的那個月印 `cache_hit` 就是踩到了；刪掉目錄重跑，應改印 `cache_miss` 並實際複製。
