...
- **規則**：**迴圈次數會跟著「欄數」長的 `withColumn`（同理 `withColumnRenamed`、`drop`）就當成 bug**，改成單次 `select`。**門檻在迴圈次數，不在「有沒有用 withColumn」**——同一個檔案裡的 `encode_categoricals` 也是 `withColumn` 迴圈，但它跑的是類別欄（個位數到十幾個），成本可以忽略——而且 #278 Out of Scope 第 8 項明訂它的語意不動，**別照本條去「順手修」它**。`io/hive_table_dataset.py` 補缺欄那段同樣是次數有界的迴圈，同理不必動。實例見 `src/recsys_tfb/preprocessing.py` 的 `cast_feature_floats_to_float32`。
