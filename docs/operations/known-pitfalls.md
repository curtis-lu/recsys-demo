...
- **`generate_report` 這個實例已修復（2026-07-20，Plan 1.5）**：修復前它是「7 個具名參數 ＋ `*args` varargs」——registry 診斷用 `*(f"evaluation_{name}" for name in DIAGNOSES)` 展開接在尾端，中間漏一個完全不會報錯，只會平移。修法是**把簽章改成剛好 8 個必填、無預設值、無 varargs 的參數**（`evaluation_metrics, parameters, baseline_metrics, metric_ci, offset_sweep, pair_ledger, report_aggregates, diagnosis_pages`），對應 `pipeline.py` 裡剛好 8 個元素的 `inputs` 列表（**2026-07-22 更新**：`offset_sweep`（PR#114）與 `pair_ledger`（Plan 3 併入 suppression）其後移除，`generate_report` 現行為 6 個必填參數＝`evaluation_metrics, parameters, baseline_metrics, metric_ci, report_aggregates, diagnosis_pages`；位置綁定機制與教訓不變）。**varargs 消失才是真正的修復**：少一個輸入現在會直接 `TypeError: missing 1 required positional argument`，在 node 呼叫當下就炸，不會等到報表算出一堆型別兜得上但語意錯的數字。純粹「參數個數對得上」不算修好——8 對 8 只是把賭注從「兩邊型別碰巧相容」降到「兩邊個數碰巧相同」，varargs 沒了才是把「漏一個不報錯」這個洞本身補起來。
...
- **驗證方式**：
  ...
  "
  ```
  （`create_pipeline()` 不帶參數——舊版寫 `create_pipeline({})`，`{}` 是 falsy，剛好被當成 `post_training=False` 的預設值用，能跑只是巧合，不要照抄。）第 4 位應為 `metric_ci` ↔ `evaluation_metric_ci`、第 5 位 `report_aggregates` ↔ `evaluation_report_aggregates`。repo 內另有 `test_inputs_positionally_match_signature` 守這件事，但**它守不了手動同步的環境**（見 §13）。
...
- **規則**：(a) 需要「本機重繪 / 驗 per-segment」前，先確認 `enriched_eval_predictions` 是富的（count 遠大於 1、且有 `cust_segment_typ` 欄）；被洗掉就重建，別把空 per-segment 誤判成程式 bug。(b) 重建＝`python -m recsys_tfb evaluation --env local --post-training --model-version <mv>`（走 `training_eval_predictions`，不需要 inference 產物；`prepare_eval_data` 會重寫這個月的分區（#352 起直接寫，原本由 `persist_eval_predictions` 寫）；segment 欄由 `prepare_eval_data` 從該模式的母體表——`--post-training` 是 `sample_pool`——依 `(time, entity)` join 進來，見 `docs/pipelines/evaluation.md` §3.2；更新於 2026-09-13，證據 #344 的 commit `357ae78`：原本寫的 `join_segment_sources` 從 `ml_recsys.sample_pool` join 是舊的設定寫死路徑）。(c) 跑重繪/驗收的 session 不要順手跑 `test_evaluation_compare_pipeline.py`；跑了就要重建。
...
- **規則**：重算既有月份用 `bash scripts/rebuild_eval_month.sh <該月>`——它把同一個 `--rebuild-dates` 同時給 dataset 與 training，training 那側再順手丟掉該月的 cache 目錄。**不要只跑其中一半**。完整動線見 [adding-an-eval-month.md](user-guides/adding-an-eval-month.md#上游回補了要重算某個月份)。單純**新增**月份不受此坑影響——新月份三層都沒有東西，必然重做；而且若忘了先跑 dataset，逐月複製的 glob 零命中會 raise `FileNotFoundError`，是 fail-loud 而非靜默。
- **驗證方式**：`ls -d data/recsys_cache/<base_dataset_version>/test_months/*` 看目前 cache 了哪幾個月。重算時該月的三行 log 應該是 `[months] dataset=test_model_input processed=<該月>`（`[months]` 那組是逐產物一行，產物名以 `pipelines/dataset/month_plans.py` 的 `INCREMENTAL_DATASETS` 為準）、`cache_rebuild name=test_model_input path=...`（緊接著 `cache_miss` 與實際複製）、`[months] predict: processed=<該月> ... rebuilt=<該月>`。該月出現在任何一行的 `skipped=` 或印成 `cache_hit`，就是那一層沒被重算。
...
- **規則**：**迴圈次數會跟著「欄數」長的 `withColumn`（同理 `withColumnRenamed`、`drop`）就當成 bug**，改成單次 `select`。**門檻在迴圈次數，不在「有沒有用 withColumn」**——同一個檔案裡的 `encode_categoricals` 也是 `withColumn` 迴圈，但它跑的是類別欄（個位數到十幾個），成本可以忽略——而且 #278 Out of Scope 第 8 項明訂它的語意不動，**別照本條去「順手修」它**。`io/hive_table_dataset.py` 補缺欄那段同樣是次數有界的迴圈，同理不必動。實例見 `src/recsys_tfb/preprocessing.py` 的 `cast_numeric_features_to_storage_type`（#283 起它的迴圈長度＝所有數值特徵欄，正是本條在講的形狀）。
