...
- [10.1 什麼時候 SQL 不夠用，該考慮 DataFrame API](#101-什麼時候-sql-不夠用該考慮-dataframe-api)
- [10.2 關鍵觀念：它們是同一個引擎，換 API 不是為了更快](#102-關鍵觀念它們是同一個引擎換-api-不是為了更快)
- [10.3 基本對照：你已經會的 SQL，換成 API 長什麼樣](#103-基本對照你已經會的-sql換成-api-長什麼樣)
- [10.4 改用時最容易踩的坑](#104-改用時最容易踩的坑)
- [10.5 可測試與重用：這才是進階主線要它的真正原因](#105-可測試與重用這才是進階主線要它的真正原因)
- [10.6 取捨與一句話帶走](#106-取捨與一句話帶走)
