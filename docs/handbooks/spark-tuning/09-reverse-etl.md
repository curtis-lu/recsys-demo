...
**對方只有 API、沒有資料庫怎麼辦？** 注意生產約束：**無外網、不在 Spark executor 裡打外部 API**（executor 數量多、無法控速、也違反 No UDF/無外網的紀律）。正確做法是**先把資料落地成檔（交換區），再由排程的一個獨立步驟**（driver 端腳本或公司內網的中介服務）把它分批、控速地推給對方 API。「先落地、再由獨立步驟推」正是為了能**控速**，配合對方系統的流量上限（rate limit）與收批時間窗——你不知道什麼時候到這筆、什麼時候超量，用落地檔當緩衝就能在推送層精確控制速率。
