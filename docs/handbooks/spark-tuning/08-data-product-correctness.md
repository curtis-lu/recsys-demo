...
- [8.1 本章地圖：把表/特徵當成「有人長期依賴的產品」](#81-本章地圖把表特徵當成有人長期依賴的產品)
- [8.2 資料品質驗證：主動抓「沒報錯的錯」](#82-資料品質驗證主動抓沒報錯的錯)
- [8.3 時間點正確性與特徵洩漏：算特徵時，絕不碰未來](#83-時間點正確性與特徵洩漏算特徵時絕不碰未來)
- [8.4 共用特徵庫的多消費者契約：改一張表＝同時改 N 個下游](#84-共用特徵庫的多消費者契約改一張表同時改-n-個下游)
- [8.5 資料版本與可重現性：沒有 time-travel，怎麼留住歷史](#85-資料版本與可重現性沒有-time-travel怎麼留住歷史)
- [8.6 進階：資料版本化的設計模式與歷史回補](#86-進階資料版本化的設計模式與歷史回補)
- [8.7 取捨總表](#87-取捨總表)
- [8.8 一句話帶走：跑成功 ≠ 算對，可信的資料產品要主動去守](#88-一句話帶走跑成功--算對可信的資料產品要主動去守)

---

## 8.1 本章地圖：把表/特徵當成「有人長期依賴的產品」

第 07 章把產出的表當成「要長期營運的服務」，盯的是它**跑不跑得起來**。這一章再進一步：你產的 `cust_feature`（**貫穿本章的例子＝每個客戶每個月底長什麼樣的特徵表，一列＝一個客戶 × 一個月**）不是你私人的中間結果，而是**好幾個模型、好幾個同事長期取用的資料產品**。產品的關鍵不是快，是**可信**，下游敢不敢閉著眼睛拿去用、三個月後出事查不查得回去。
...
**原則（特徵庫的頭號風險）。** 先定錨兩個詞：**特徵（feature）**＝描述每個客戶的一個數字／屬性（近 30 天消費、目前餘額…），模型拿它來預測；**label**＝你要模型預測的那個答案（會不會買、會不會違約）。訓練模型時，每一筆特徵都該回答一個問題：「**在要做預測的那個時間點，這個資訊真的拿得到嗎？**」如果你不小心用了「預測當下還不可能知道的資訊」去當特徵，就是**特徵洩漏（data leakage）**。它的可怕之處是：離線評估會**虛高到爆**（因為偷看了答案），但一上線、真的只能用當下資訊時，模型就**原形畢露**。第 07 章 §7.8 最後一列「測得很準、上線很差」講的就是它。
...
- 想看「我這類工作（ad-hoc／排程產表／特徵運算）通常照哪些章、最常踩什麼雷」的索引？→ 見[首頁〔場景對應〕](index.md#場景對應先認出你在做哪種工作)。
...
*←上一章* [07 · 營運（一）：可靠地把排程跑起來](07-operating-pipelines.md)　|　*下一章 →* [09 · 營運（三）：把資料送出去——reverse ETL 回業務系統](09-reverse-etl.md)　|　*回* [手冊首頁](index.md)
