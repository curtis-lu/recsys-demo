...
- [6.1 本章地圖：同一份資料，三個引擎都能查](#61-本章地圖同一份資料三個引擎都能查)
- [6.2 三個引擎是怎麼跑你的查詢的](#62-三個引擎是怎麼跑你的查詢的)
- [6.3 怎麼選：一張決策表](#63-怎麼選一張決策表)
- [6.4 各引擎怎麼看「為什麼慢」：診斷工具對照](#64-各引擎怎麼看為什麼慢診斷工具對照)
- [6.5 CDP 實務一：三引擎共用同一張表，威力與陷阱](#65-cdp-實務一三引擎共用同一張表威力與陷阱)
- [6.6 CDP 實務二：Impala 看不到新資料？`REFRESH` 與 `INVALIDATE METADATA`](#66-cdp-實務二impala-看不到新資料refresh-與-invalidate-metadata)
- [6.7 CDP 實務三：ACID／交易表跨引擎的限制](#67-cdp-實務三acid交易表跨引擎的限制)
- [6.8 取捨講白：每個引擎的甜蜜點與雷區](#68-取捨講白每個引擎的甜蜜點與雷區)
- [6.9 把它全部串起來：一天裡，三個引擎各司其職](#69-把它全部串起來一天裡三個引擎各司其職)
- [6.10 一句話帶走：同一份資料，對的工作交給對的引擎](#610-一句話帶走同一份資料對的工作交給對的引擎)
...
- 想看「我這類工作（ad-hoc／排程／特徵）通常照哪些章、用哪個引擎、最常踩什麼雷」？→ 見[首頁場景速查](index.md#場景速查依你的工作型態)。
