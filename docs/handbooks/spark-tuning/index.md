...
- **要把表/特徵變成長期營運、給別人用的資料產品** → 第 08 章（冪等、回填、品質驗證、時間點正確性、監控、維護）。
- **想看自己這類工作（ad-hoc / 排程產表 / 特徵運算）照哪些章** → 第 09 章。
- **想快速查一個參數預設值、或某個名詞是什麼** → 第 10 章。

> **兩條主線**：只做分析查詢 → 讀 01–06 ＋ 第 10 章速查就很夠；要**營運排程／特徵庫** → 再往下讀 07（視需要）、**08（營運）**、09（場景索引）。
...
| 08 | [營運資料排程與資料產品](08-operating-data-pipelines.md) | 把表/特徵當服務營運：冪等可重跑、回填、資料品質驗證、時間點正確性、監控、表維護 |
| 09 | [場景對應（索引）](09-scenario-playbooks.md) | ad-hoc 分析 / 排程產表 / 特徵運算，各自照哪些章、踩什麼雷 |
| 10 | [速查與名詞表](10-cheatsheet-and-glossary.md) | 取捨速查、config 速查、中英名詞對照 |
