...
def _stream_matrix(
    ...
    ``on_disk=True`` maps the matrix from a scratch file instead of allocating
    it on the heap (:mod:`recsys_tfb.io.disk_matrix`). It changes where the
    bytes live and nothing else — same dtype, same batching, byte-identical
    contents. It is a caller's declaration ("I am going to hold this for a
    long time"), never a size test: a branch that only fires on data large
    enough to hurt is a branch no test ever reaches.
    ...
    **Quote the second row.** The training cache is the partitioned tree
    ...
    X = (
        disk_matrix.open_disk_matrix(shape, dtype, log_prefix)
        if on_disk
    ...
    logger.info(
        ...
        batch_rows, X.nbytes / 1024**2, on_disk,
    )
...
def extract_Xy_with_groups(
    ...
    *,
    ...
    ``on_disk=True`` returns X mapped from a scratch file rather than held on
    the heap — byte-identical either way, see :func:`_stream_matrix`. Only
    ``tune_hyperparameters`` asks for it: it is the one caller that holds a
    matrix across every fit of the search, so its resident memory otherwise
    grows with the val row count. The ``.bin`` prep, the refit and the
    calibration read each hold theirs for a single fit and are left alone,
    which is also why :func:`extract_Xy` has no such flag — none of its
    callers would pass it.
    """
    feature_cols = preprocessor_metadata["feature_columns"]
    schema = get_schema(parameters)
    label_col = schema["label"]
    ...
    X, aux = _stream_matrix(
        ...
        "extract_Xy_with_groups", on_disk=on_disk,
    )
    y = aux[label_col].values
