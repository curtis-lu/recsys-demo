...
def iter_stratified_cluster_multipliers(
    ...
    **為什麼是 generator 而不是回一個 (n_boot, n_rows) 矩陣**：公司規模是
    ...
    n_clusters = int(clusters.max()) + 1
    unit_code = stratum_codes.astype(np.int64) * n_clusters + clusters
    ...
    for _ in range(n_boot):
        ...
        for s in range(len(uniq_strata)):
            ...
            mult += np.bincount(picks, minlength=n_units).astype(np.float64)
        yield mult[unit_inv]
...
def paired_bootstrap_delta(
    ...
    3. **重抽單位是 ``(stratum, cluster)`` 而非 cluster**：stratum 是
       ...
       **單一日期評估時這個代價是惰性的**：query ＝ ``(snap_date, entity)``，
       一次只評估一個 snap_date 時，一個 entity 恰好對應一個 query、恰好屬於
       一個層，跨層不可能發生（2026-07-19 在本機資料實測：654 個 entity、
       654 個 query，比值 1.000）。
       ⚠ **evaluation 現在可以一次評估多個日期（#374，``evaluation.snap_date``
       寫成清單或區間），這個取捨在那種執行裡會活過來**——同一個 entity 的
       不同日期可能落在不同層，被當成兩個獨立 cluster，低估變異、CI 偏窄。
       #374 只在主報表 metadata 與文件加警告；修正追蹤於 issue #389。
    ...
    # 重抽單位 ＝ (stratum, cluster)，編碼成單一 int。這裡的 unit_code／
    # strata_units 是 iter_stratified_cluster_multipliers 內部也會算的同一份
    # 靜態對應——不是重抽本身，只是「哪個 unit 屬於哪一層」的查表，供下面
    # _record_draws 測試 hook 從回傳的 mult 向量反查每層抽到幾個單位用。
    # 實際的分層重抽（rng.integers 呼叫序列）完全交給該函式，這裡不再重寫。
    uniq_strata, stratum_codes = np.unique(strata, return_inverse=True)
    ...
    for b, mult_row in enumerate(
        iter_stratified_cluster_multipliers(cluster_codes, strata, n_boot, seed)
    ):
        if _record_draws is not None:
            # mult_row 是列空間，同一個 unit 的所有列取值相同——任取一列即可
            # 還原該 unit 的抽中次數，不需要另外呼叫 RNG。
            unit_val = np.empty(n_units, dtype=np.float64)
            unit_val[unit_inv] = mult_row
            _record_draws.append({
                str(uniq_strata[s]): int(unit_val[strata_units[s]].sum())
                for s in range(len(uniq_strata))
            })

        w_row = base_w * mult_row
